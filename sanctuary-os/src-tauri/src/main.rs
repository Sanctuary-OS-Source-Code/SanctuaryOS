...
fn check_for_malware_poc(path: &Path) -> bool {
    if let Ok(file) = std::fs::File::open(path) {
        if let Ok(mut archive) = zip::ZipArchive::new(file) {
            for i in 0..archive.len() {
                if let Ok(file) = archive.by_index(i) {
                    if file.name().to_lowercase().contains("ts4_poc.py") {
                        return true;
                    }
                }
            }
...
fn scan_bunker(
    ...
    for path in paths {
        ...
        } || (is_script && check_for_malware_poc(&path));
        ...
        if is_malware {
            ...
            results.push(ModData {
                name: rel.replace("\\", "/"),
                hash: dna_hash,
                status: "Γÿú∩╕Å QUARANTINED".to_string(),
        ...
        let status_string = if explicitly_local {
            "≡ƒÜ½ EXPLICIT LOCAL".to_string()
        } else {
            "≡ƒöÿ LOCAL_ONLY".to_string()
        };
