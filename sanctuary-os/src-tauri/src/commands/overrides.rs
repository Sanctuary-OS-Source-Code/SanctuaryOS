    ...
    if let Some(parent) = source_original.parent() {
        ...
        if current_location.exists() {
            fs::rename(&current_location, &source_original)
                .or_else(|_| {
                    fs::copy(&current_location, &source_original)
                        .and_then(|_| fs::remove_file(&current_location))
                })
...
pub fn clear_overrides(mods_path: String) -> Result<String, String> {
        ...
        .filter_map(|e| e.ok())
    {
        if entry.file_type().is_dir() && entry.file_name() == "Sanctuary" {
            for sub_entry in walkdir::WalkDir::new(entry.path())
                .into_iter()
                .filter_map(|e| e.ok())
            {
                if sub_entry.file_type().is_file() {
                    ...
                    if let Ok(_) = fs::rename(sub_entry.path(), &dest).or_else(|_| {
                        fs::copy(sub_entry.path(), &dest)
                            .and_then(|_| fs::remove_file(sub_entry.path()))
                    }) {
                        count += 1;
                    }
                }
