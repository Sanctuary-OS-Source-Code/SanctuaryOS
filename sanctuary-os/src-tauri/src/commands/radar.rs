...
pub async fn run_conflict_radar(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let installed_files: Vec<String> = packages
            .iter()
            .map(|path| {
                ...
                for key in resources {
                    if let Some(clash) = resource_map.get(&key) {
                        if clash != &file_name {
                            ...
                            let (_, rank) =
                                crate::game_logic::get_severity_rank(&game_schema_merge, key.0);
        ...
        packages.into_par_iter().for_each_with(tx, |s, path| {
            let file_name = path
            ...
            if let Ok(file_resources) = crate::dbpf::read_dbpf_index(&path) {
                for res in file_resources {
                    let (is_harmless, _) =
                        crate::game_logic::get_severity_rank(&game_schema, res.t);
                    if !is_harmless {
