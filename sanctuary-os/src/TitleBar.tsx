...
export function TitleBar({ isSidebarCollapsed, setIsSidebarCollapsed, subtitleIndex }: any) {
  ...
  const activeGameName = activeGame?.name || activeWorkspace?.name || activeGameSchema?.name || "Sanctuary OS";
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 h-[70px] pointer-events-none flex items-center z-[999998] px-6">
        ...
        <div className="shrink-0 transition-all duration-500 hidden md:block" style={{ width: isSidebarCollapsed ? '80px' : 'var(--sidebarWidth, 288px)' }} />

        {/* Center Game Selector Pill */}
        <div className="flex-1 flex justify-center h-full items-center">
          {workspaces?.length > 0 ? (
            <button
              className="pointer-events-auto glass-panel rounded-full px-5 py-2 flex items-center gap-3 cursor-pointer group/launcher hover:bg-[color-mix(in_srgb,var(--text)_5%,transparent)] transition-colors duration-300 shadow-lg border border-[color-mix(in_srgb,var(--text)_10%,transparent)]"
            ...
            <div className="pointer-events-none glass-panel rounded-full px-5 py-2 flex items-center gap-3 shadow-lg border border-[color-mix(in_srgb,var(--text)_10%,transparent)]">
              <span className="material-symbols-outlined !text-[18px] opacity-50 drop-shadow-md">sports_esports</span>
              <span className="text-[14px] tracking-wide font-black capitalize text-[var(--text)] opacity-90 px-1 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                {activeGameName}
              </span>
            </div>
          )}
