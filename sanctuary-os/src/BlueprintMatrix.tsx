...
export default function BlueprintMatrix({ isOpen, onClose, playSet, modList, onUpload }: any) {
  ...
  return createPortal(
    <SidePanel
      isOpen={isOpen}
      onClose={onClose}
      title={t("matrix_title") || "BLUEPRINT MATRIX"}
      subtitle={t("matrix_subtitle") || "Configure Deployment Protocol"}
      footer={
        <div className="flex justify-center items-center gap-4 w-full">
          ...
          <button onClick={handleUpload} disabled={isUploading} className={standardAccentGlassButtonClass}>
            {isUploading ? (t("cmd_scanning") || "GENERATING...") : (t("matrix_btn_generate") || "GENERATE MATRIX")}
          </button>
        </div>
      }
    >
