...
export const supabaseServices = {
  ...
  getCommunityTemplate: async (targetFile: string) => {
    const { data, error } = await supabase.from('marketplace_assets').select('*').eq('asset_type', 'workbench_template').eq('is_community_default', true);
  ...
  getAllCommunityDefaults: async () => {
    const { data, error } = await supabase.from('marketplace_assets').select('id, json_data, author').eq('asset_type', 'workbench_template').eq('is_community_default', true);
  ...
  getWorkbenchTemplates: () => supabase.from('marketplace_assets').select('*').eq('asset_type', 'workbench_template').order('created_at', { ascending: false }),
  setCommunityDefaultTemplate: async (id: string, targetFile: string) => {
    const { data } = await supabase.from('marketplace_assets').select('id, json_data').eq('asset_type', 'workbench_template').eq('is_community_default', true);
    if (data) {
        for (const item of data) {
            try {
                const parsed = JSON.parse(item.json_data);
                if (parsed.target_file === targetFile && item.id !== id) {
                    await supabase.from('marketplace_assets').update({ is_community_default: false }).eq('id', item.id);
                }
            } catch (e) {}
        }
    }
    return supabase.from('marketplace_assets').update({ is_community_default: true }).eq('id', id);
  },
