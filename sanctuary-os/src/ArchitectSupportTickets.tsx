...
export default function ArchitectSupportTickets({ userRole = "architect", masonProfileId, onOpenDNA, setStatus }: { userRole?: string, masonProfileId?: string, onOpenDNA?: (hash: string) => void, setStatus?: any }) {
      ...
      if (userRole !== "mason") {
         finalTickets = mergedTickets.filter(t => {
            ...
            if (userRole === 'wayfinder') {
               ...
               // Treat 'Mason' as a potential Senior Architect action due to prior logging bug
  ...
  const handleTakeAction = async (actionType: "RESOLVED" | "REJECTED" | "ESCALATED" | "PENDING", reason: string) => {
    ...
    // Add the action reason to the metadata
    const newMetadata = {
      ...
      action_log: [
        ...(selectedTicket.metadata?.action_log || []),
        {
          ...
          architect: userRole === "senior_architect" ? "Senior Architect" : userRole === "wayfinder" ? "Wayfinder" : userRole === "architect" ? "Architect" : "Mason"
        }
      ]
    };
    ...
    <div className="flex flex-col gap-6 w-full pb-32 text-[var(--text)]">
      <div className="flex items-center gap-4 px-6 py-4 shrink-0 border-b border-white/5 w-full mb-4">
        ...
        <div className="flex items-center gap-3 relative flex-1 max-w-2xl ml-auto justify-end">
          ...
          <div className="w-48 shrink-0 relative z-50 h-12">
            <CustomDropdown 
