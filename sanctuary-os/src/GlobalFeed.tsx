...
if (!(window as any).__sanctuaryCache.globalFeed) {
  (window as any).__sanctuaryCache.globalFeed = {
    posts: null,
    lastFetch: 0
  };
}
    ...
    if (isOffline) return;
      if (activeTab === "OVERVIEW") {
        const fetchStats = async () => {
          ...
          const { data: masonData } = userId ? await supabase.from('masons').select('id').eq('profile_id', userId).maybeSingle() : { data: null };
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      ...
      if (activeTab === "FOLLOWING") {
        ...
        if (currentUserId) {
          const { data: followData } = await supabase.from('mason_followers').select('mason_id').eq('user_id', currentUserId);
          followedIds = followData?.map(f => f.mason_id) || [];
        }
        let data, error;
        if (followedIds.length > 0) {
          ({ data, error } = await supabase.from('mason_posts').select('*, masons(*), likes:mason_post_likes(count), views:mason_post_views(count), comments:mason_post_comments(count)').in('mason_id', followedIds).order('created_at', { ascending: false }).limit(4));
        } else {
          ({ data, error } = await supabase.from('mason_posts').select('*, masons(*), likes:mason_post_likes(count), views:mason_post_views(count), comments:mason_post_comments(count)').order('created_at', { ascending: false }).limit(4));
        }
        if (error) console.error(error);
        if (data) setPosts(data);
      } else {
