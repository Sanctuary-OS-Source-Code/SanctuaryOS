...
export default function MasonProfileHeader({ mason, masonId, followerCount, isFollowing, masonAlerts, toggleFollow, toggleMasonAlert, activeView, setActiveView, t }: any) {
  return (
    <div className="relative mb-0 flex flex-col w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="relative p-8 lg:p-12 group rounded-3xl border border-[color-mix(in_srgb,var(--text)_5%,transparent)] overflow-hidden shadow-2xl backdrop-blur-2xl bg-[color-mix(in_srgb,var(--bg)_30%,transparent)]">

        {/* Dynamic Holographic Backgrounds */}
        <div className="absolute inset-0 bg-gradient-to-br from-[color-mix(in_srgb,var(--accent)_5%,transparent)] via-transparent to-[color-mix(in_srgb,var(--bg)_60%,transparent)] z-0 pointer-events-none" />
        <div className="absolute -top-[20rem] -right-[20rem] w-[50rem] h-[50rem] theme-bg-accent opacity-[0.03] blur-[120px] rounded-full pointer-events-none group-hover:opacity-[0.05] transition-opacity duration-1000 z-0" />
        <div className="absolute -bottom-[10rem] -left-[10rem] w-[30rem] h-[30rem] bg-[var(--text)] opacity-[0.02] blur-[100px] rounded-full pointer-events-none z-0" />

        {/* Tech Grid Overlay */}
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MCIgaGVpZ2h0PSI0MCI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMjU1LDI1NSwyNTUsMC4wNSkiLz48L3N2Zz4=')] opacity-10 pointer-events-none z-0 mix-blend-overlay" />
        ...
        <div className="relative z-10 grid grid-cols-1 xl:grid-cols-[auto_1fr_auto] gap-10 items-center">
            ...
            <div className="relative w-36 h-36">
              ...
              <div className="absolute inset-0 rounded-2xl bg-[color-mix(in_srgb,var(--sidebar)_80%,transparent)] border border-[color-mix(in_srgb,var(--text)_10%,transparent)] flex items-center justify-center overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-10 backdrop-blur-xl group/avatar cursor-pointer hover:border-[color-mix(in_srgb,var(--accent)_40%,transparent)] transition-all duration-500">
                ...
                {mason.avatar_url ? (
                  ...
                  <span className="text-6xl opacity-30 grayscale material-symbols-outlined">{t("icon_construction")}</span>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap items-end gap-4">
                <div className="flex items-center gap-3">
                  <h1
                    className={`text-4xl lg:text-5xl font-black capitalize tracking-tight text-[var(--text)] drop-shadow-[0_0_15px_color-mix(in_srgb,var(--text)_10%,transparent)] transition-colors ${setActiveView ? 'cursor-pointer hover:theme-text-accent' : ''}`}
                  ...
                  {mason.is_verified && (
                    <div className="flex items-center gap-2 mb-1" title={t("verified")}>
                      <span className="material-symbols-outlined !text-[28px] text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]">{t("icon_verified_user")}</span>
                    </div>
                  )}
                </div>
                ...
                {activeView && activeView !== 'OVERVIEW' && (
                  <div className="flex items-center gap-3 mb-1 ml-2">
                    <span className="text-[var(--subtext)] opacity-30 font-black text-3xl leading-none">/</span>
                    <span className="text-2xl font-black tracking-widest uppercase text-[var(--accent)] animate-in fade-in slide-in-from-left-2 duration-300 drop-shadow-[0_0_10px_color-mix(in_srgb,var(--accent)_30%,transparent)]">
          ...
          <div className="flex flex-col gap-4 w-full xl:w-[320px]">
            ...
            <button onClick={toggleFollow} className={`relative group/mainbtn w-full h-14 rounded-2xl flex items-center justify-center gap-3 text-xs font-black uppercase tracking-[0.2em] transition-all duration-300 overflow-hidden shadow-xl ${isFollowing ? 'bg-[color-mix(in_srgb,var(--text)_5%,transparent)] border border-[color-mix(in_srgb,var(--text)_10%,transparent)] text-[var(--text)] hover:bg-[color-mix(in_srgb,var(--text)_10%,transparent)]' : 'bg-[color-mix(in_srgb,var(--accent)_15%,transparent)] border border-[color-mix(in_srgb,var(--accent)_30%,transparent)] text-[var(--accent)] hover:border-[var(--accent)] hover:shadow-[0_0_30px_color-mix(in_srgb,var(--accent)_30%,transparent)] hover:-translate-y-1'}`}>
              ...
              <span className="material-symbols-outlined !text-[20px] relative z-10">{isFollowing ? 'check_circle' : 'person_add'}</span>
