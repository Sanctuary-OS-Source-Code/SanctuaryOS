...
export default function Marketplace({ ownedHashes, onSetStatus, onOpenMasonProfile, onOpenDossier, syncBlueprintByCode }: MarketplaceProps) {
  ...
  const isInstalled = (asset: any) => {
    if (marketTab === 'CHAMELEONS') {
      ...
      // Just check if there's any file named like the template ID
      return false; // Can't accurately test if template file is installed from here easily without hitting disk
    } else {
                          ...
                          <button 
                            onClick={async (e) => {
                              ...
                              if (marketTab === 'CHAMELEONS') {
                                ...
                                await importTemplate(asset.json_data);
