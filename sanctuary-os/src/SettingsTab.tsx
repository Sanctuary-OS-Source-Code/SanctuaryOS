          ...
          >
            {options.map((opt: any) => (
              <button key={opt.id} onClick={() => { onChange(opt.id); setIsOpen(false); }} className={`w-full text-left px-5 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b border-[color-mix(in_srgb,var(--text)_5%,transparent)] last:border-0 flex items-center justify-between group/opt hover:bg-[color-mix(in_srgb,var(--text)_5%,transparent)] hover:pl-7 ${opt.id === value ? 'theme-text-accent bg-[color-mix(in_srgb,var(--accent)_10%,transparent)]' : 'text-[var(--text)]'}`}>
                {opt.label}
                {opt.id === value && <span className="material-symbols-outlined !text-[16px] text-[var(--accent)]">{t("ui_icon_check")}</span>}
              </button>
            ))}
          </div>
        </>,
...
export default function Settings({ anarchyRules, setAnarchyRules }: any) {
  ...
  const executeOverride = async () => {
    ...
    useStore.getState().pushStatus(t("settings_malware_severed_alert"));
  ...
  const TABS = [
    { id: 'SYSTEM', icon: t("ui_icon_pin"), label: t("settings_tab_system") },
    { id: 'AUTH', icon: t("ui_icon_lock"), label: t("settings_tab_auth") },
    { id: 'NETWORK', icon: t("ui_icon_globe"), label: t("settings_tab_network") },
    { id: 'NOTIFICATIONS', icon: t("ui_icon_notifications"), label: t("settings_tab_notifs") },
    { id: 'CHAMELEON', icon: t("ui_icon_theme"), label: t("settings_tab_chameleon") },
    { id: 'LEXICON', icon: t("ui_icon_language"), label: t("settings_tab_lexicon") },
    { id: 'LOGIC', icon: t("ui_icon_flag"), label: t("settings_tab_logic") }
  ];
  if (showMalwareTab) {
    TABS.push({ id: 'MALWARE', icon: t("ui_icon_warning"), label: t("settings_tab_malware") });
  }
  ...
  return (
    <div className="flex flex-col h-full w-full p-8 gap-8 animate-in fade-in duration-500 overflow-y-auto custom-scrollbar pb-[30rem]" style={{ color: currentTheme.text }}>
      ...
      >
        ...
        >
          <span className="material-symbols-outlined !text-lg">{t("ui_icon_logout")}</span> {t("settings_btn_logout")}
        </button>
      </ViewHeader>
      ...
      <div className="flex flex-1 gap-16 mt-2 relative">
        ...
        <div className="flex-1 relative pr-6">
           ...
           {activeTab === 'SYSTEM' && (
               ...
               <TabContainer 
                  ...
                  actions={
                    <button onClick={async () => {
                      ...
                      <span className="material-symbols-outlined !text-lg text-[var(--accent)] drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]">{t("ui_icon_troubleshoot")}</span>
                     ...
                     <div key={dir.rustKey} className="group relative" onMouseEnter={() => setHoveredKey(dir.rustKey)} onMouseLeave={() => setHoveredKey(null)}>
                       ...
                       <div onClick={() => pickPath(dir.rustKey, dir.label)} className="relative flex items-center cursor-pointer group/input hover:scale-[1.01] transition-transform">
                         ...
                         <div className="absolute right-3 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-300 theme-glass-inner border border-white/20 hover:scale-105 active:scale-95 flex items-center gap-2 shadow-lg hover:theme-border-accent bg-black/40 text-[var(--text)]">
                            <span className="material-symbols-outlined !text-sm text-[var(--accent)]">{t("ui_icon_sync")}</span>
             ...
             <TabContainer 
  title={t("settings_timeline_versions")}
  icon="history"
>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full">
      ...
      <div className="group">
        ...
        <CustomSettingsDropdown 
          ...
          options={[
            { id: 10, label: t("settings_timeline_10") },
            { id: 50, label: t("settings_timeline_50") },
            { id: 100, label: t("settings_timeline_100") },
            { id: 500, label: t("settings_timeline_500") }
          ]} 
        />
      </div>
      ...
      <div className="group">
        <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">
          {t("settings_timeline_size")}
        </label>
        <CustomSettingsDropdown 
          value={config.timeline_retention_size_mb || 100} 
          onChange={(val: any) => updateConfig('timeline_retention_size_mb', val)}
          options={[
            { id: 50, label: t("settings_timeline_size_50") },
            { id: 100, label: t("settings_timeline_size_100") },
            { id: 500, label: t("settings_timeline_size_500") },
            { id: 1024, label: t("settings_timeline_size_1024") }
          ]} 
        />
      </div>
               ...
               >
                 <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
                  <div className="flex flex-col gap-8">
                    ...
                    <div className="group">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_vault_capacity")}</label>
                      <CustomSettingsDropdown 
                        value={config.vault_capacity_gb || 0} 
                        onChange={(val: any) => updateConfig('vault_capacity_gb', val)}
                        options={[
                          { id: 0, label: t("settings_capacity_unlimited") },
                          { id: 10, label: t("settings_capacity_10") },
                          { id: 25, label: t("settings_capacity_25") },
                          { id: 50, label: t("settings_capacity_50") },
                          { id: 100, label: t("settings_capacity_100") }
                        ]} 
                      />
                    </div>
                  </div>
                   ...
                   <div className="flex flex-col gap-3 group">
                     ...
                     <div className="flex gap-2">
                       <input 
                         ...
                         placeholder={t("auto_mason_sanctuary_network")}
             ...
             <TabContainer title={t("settings_tab_network")} icon={TABS.find(t=>t.id==='NETWORK')?.icon}>
               <div className="grid xl:grid-cols-2 gap-12">
                   ...
                   <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={() => setShowImages(!showImages)}>
                      <div className="flex flex-col gap-2">
                        <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_show_images")}</span>
                        <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_show_images_desc")}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={() => {
                      ...
                      <div className="flex flex-col gap-2">
                        <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_malware_share_title")}</span>
                        <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_malware_share_desc")}</span>
                      </div>
             ...
             <TabContainer 
                ...
                actions={
                  <button onClick={() => setShowMasonPanel(true)} className={standardButtonClass}>
                    <span className="material-symbols-outlined lowercase theme-text-accent text-lg">{t("emote_bell")}</span> {t("settings_notif_per_mason")}
                  </button>
                }
             >
             ...
             >
                 ...
                 {Object.entries({ ...CORE_THEMES, ...customThemes }).map(([id, data]: any) => (
                   <button key={id} onClick={() => setActiveThemeId(id)} className={`p-6 rounded-3xl border transition-all text-left group relative backdrop-blur-xl shadow-lg ${activeThemeId === id ? 'theme-border-accent theme-glass-inner shadow-[0_0_30px_rgba(var(--accent-rgb),0.2)]' : 'theme-glass-panel opacity-80 hover:opacity-100'}`}>
                     ...
                     <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                       <div onClick={(e) => handleExportTheme(e, data)} className="p-2 rounded-full hover:bg-white/10 text-sm material-symbols-outlined lowercase">{t("ui_icon_save")}</div>
                       {!CORE_THEMES[id] && (
                         <>
                           <div onClick={(e) => { e.stopPropagation(); setNewThemeName(data.name); setEditingThemeId(id); }} className="p-2 rounded-full hover:bg-white/10 text-sm theme-text-accent material-symbols-outlined lowercase">{t("ui_icon_edit")}</div>
                           <div onClick={(e) => { e.stopPropagation(); deleteTheme(id); }} className="p-2 rounded-full hover:bg-white/10 text-sm theme-text-danger material-symbols-outlined lowercase">{t("ui_icon_delete")}</div>
                         </>
                       )}
                     </div>
                   </button>
               ...
               <div className="mt-8 pt-10 border-t border-white/5">
                  <h3 className="text-xl font-black uppercase tracking-widest text-[var(--text)] mb-8 flex items-center gap-3">
                     <span className="material-symbols-outlined text-2xl theme-text-accent">{t("ui_icon_tune")}</span>
               ...
               <div className="flex flex-col gap-12">
                 ...
                 <div className="flex flex-col gap-6 mt-8 pt-10 border-t border-white/5">
                    <h3 className="text-xl font-black uppercase tracking-widest text-[var(--text)] mb-2 flex items-center gap-3">
                       <span className="material-symbols-outlined text-2xl theme-text-accent">{t("ui_icon_menu_book")}</span>
                       {t("settings_library")}
                    </h3>
           ...
           {activeTab === 'MALWARE' && (
             <TabContainer title={t("settings_tab_malware")} icon={TABS.find(t=>t.id==='MALWARE')?.icon}>
               <div className="flex flex-col items-center justify-center p-16 text-center border border-red-500/20 rounded-[3rem] bg-red-950/20 backdrop-blur-xl shadow-[0_0_50px_rgba(239,68,68,0.1)] mt-8">
                 <span className="!text-[8rem] animate-bounce drop-shadow-[0_0_20px_rgba(220,38,38,1)] mb-8 material-symbols-outlined">{t("ui_icon_skull")}</span>
                 <p className="text-xs font-black text-red-400 opacity-80 uppercase tracking-widest max-w-xl leading-relaxed mb-12">
                   {t("settings_god_mode_warn")}
                 </p>
      ...
      <SidePanel 
        ...
        title={t("settings_notif_per_mason")}
      ...
      >
        <div className="flex-1 flex flex-col gap-8 relative">
          ...
          <div className="flex flex-col gap-4 min-w-0 flex-1 relative z-10 text-center mb-8">
            <span className="material-symbols-outlined !text-[8rem] animate-bounce drop-shadow-[0_0_20px_rgba(220,38,38,1)] mb-4 mt-8">{t("ui_icon_skull")}</span>
