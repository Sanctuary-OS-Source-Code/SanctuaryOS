          ...
          >
            {options.map((opt: any) => (
              <button key={opt.id} onClick={() => { onChange(opt.id); setIsOpen(false); }} className={`w-full text-left px-5 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b border-[color-mix(in_srgb,var(--text)_5%,transparent)] last:border-0 flex items-center justify-between group/opt hover:bg-[color-mix(in_srgb,var(--text)_5%,transparent)] hover:pl-7 ${opt.id === value ? 'theme-text-accent bg-[color-mix(in_srgb,var(--accent)_10%,transparent)]' : 'text-[var(--text)]'}`}>
                {opt.label}
                {opt.id === value && <span className="material-symbols-outlined !text-[16px] text-[var(--accent)]">{t("ui_icon_check") || "check"}</span>}
              </button>
            ))}
          </div>
        </>,
...
export default function Settings({ anarchyRules, setAnarchyRules }: any) {
  ...
  const getLexiconMetadata = (code: string) => {
    if (code === 'en-sanctuary') return { language: 'English', name: t("settings_lang_sanctuary") || "English: Sanctuary"?.split(': ')[1] || 'Sanctuary' };
    if (code === 'en-default') return { language: 'English', name: t("settings_lang_standard") || "English: Default"?.split(': ')[1] || 'Default' };
    if (code === 'de-default') return { language: 'German', name: t("settings_lang_german") || "German: Default"?.split(': ')[1] || 'Default' };
  ...
  const handleExportTheme = async (e: React.MouseEvent, themeObj: any) => {
    ...
    } catch (err) { useStore.getState().pushStatus(`${t("alert_export_failed") || "Export Failed:"}${err}`); }
  };
  ...
  const handleImportTheme = async () => {
    ...
    } catch (err) { useStore.getState().pushStatus(`${t("alert_import_failed") || "Import Failed:"}${err}`); }
  };
  ...
  const handleImportLexicon = async () => {
    try {
      ...
      if (!selected) return;
      const content = await readTextFile(selected as string);
      const langCode = prompt(t("prompt_lang_code") || "Enter Language Code (e.g. 'en-cyborg', 'es'):") || "custom";
      importLexicon(JSON.parse(content), langCode);
    } catch (err) { useStore.getState().pushStatus(`${t("alert_lexicon_failed") || "Lexicon Import Failed:"}${err}`); }
  };
  ...
  const updateAuth = async (type: 'email' | 'password') => {
    ...
    try {
      if (type === 'email' && emailInput) {
        ...
        useStore.getState().pushStatus("Email update initiated. Check your inboxes for confirmation links.");
        setEmailInput("");
      }
      if (type === 'password' && passwordInput) {
        const { error } = await supabase.auth.updateUser({ password: passwordInput });
        if (error) throw error;
        useStore.getState().pushStatus("Password updated successfully.");
        setPasswordInput("");
      }
    } catch (err: any) {
      useStore.getState().pushStatus(`Error updating credentials: ${err.message}`);
    }
    setAuthLoading(false);
  };

  const executeOverride = async () => {
    if (overrideInput.toLowerCase() !== (t("settings_malware_confirm_text") || "I Confirm").toLowerCase()) return;
    ...
    useStore.getState().pushStatus(t("settings_malware_severed_alert") || "You have chosen to override the Malware Protocol. Your Database Access has been severed.");
  ...
  const pickPath = async (rustKey: string, label: string) => {
    try {
      const selected = await open({ directory: true, multiple: false, title: `${t("modal_select_path") || "Select Directory for"} ${label}` });
      if (selected) {
        const payload = {
  ...
  if (!config) return <div className="p-12 font-black animate-pulse uppercase tracking-widest" style={{ color: currentTheme.accent }}>{t("settings_booting") || "Booting core systems..."}</div>;

  const pathMap =[
    { rustKey: 'vault_path', label: t("settings_vault_path") || "The Vault Path", value: config.vault_path, icon: t("ui_icon_collection") || "inventory_2" },
    { rustKey: 'mods_path', label: t("settings_library_path") || "Active Library (Mods Folder)", value: config.mods_path, icon: t("ui_icon_folder") || "folder" },
    { rustKey: 'live_path', label: t("setup_btn_bin") || "SELECT BIN FOLDER", value: config.live_path, icon: t("ui_icon_pin") || "push_pin" }
  ];

  const TABS = [
    { id: 'SYSTEM', icon: t("ui_icon_pin") || "push_pin", label: t("settings_tab_system") || "System Settings" },
    { id: 'AUTH', icon: t("ui_icon_lock") || "lock", label: t("settings_tab_auth") || "Identity" },
    { id: 'NETWORK', icon: t("ui_icon_globe") || "public", label: t("settings_tab_network") || "Network" },
    { id: 'NOTIFICATIONS', icon: t("ui_icon_notifications") || "notifications", label: t("settings_tab_notifs") || "Notifications" },
    { id: 'CHAMELEON', icon: t("ui_icon_theme") || "palette", label: t("settings_tab_chameleon") || "Chameleon" },
    { id: 'LEXICON', icon: t("ui_icon_language") || "language", label: t("settings_tab_lexicon") || "Lexicon" },
    { id: 'LOGIC', icon: t("ui_icon_flag") || "flag", label: t("settings_tab_logic") || "Logic Integrity" }
  ];
  if (showMalwareTab) {
    TABS.push({ id: 'MALWARE', icon: t("ui_icon_warning") || "warning_amber", label: t("settings_tab_malware") || "Malware Protocol" });
  }
  ...
  return (
    <div className="flex flex-col h-full w-full p-8 gap-8 animate-in fade-in duration-500 overflow-y-auto custom-scrollbar pb-[30rem]" style={{ color: currentTheme.text }}>
      ...
      {/* FULL WIDTH HEADER */}
      <ViewHeader 
        title={t("settings_title") || "System Preferences"} 
        subtitle={<span className="cursor-default" onClick={() => setHackerClicks(prev => prev + 1)}>{t("settings_subtitle") || "Local configuration, identity, libraries, and security protocols"}</span>} 
        icon="settings" 
      ...
      >
        ...
        >
          <span className="material-symbols-outlined !text-lg">{t("ui_icon_logout") || "logout"}</span> {t("settings_btn_logout") || "LOG OUT"}
        </button>
      </ViewHeader>
        ...
        <div className="flex-1 relative pr-6">
           ...
           {activeTab === 'SYSTEM' && (
             <div className="flex flex-col gap-8 w-full">
               <TabContainer 
                  title={t("settings_sys_coords") || "System Coordinates"}
                  ...
                  actions={
                    <button onClick={async () => {
                      ...
                      <span className="material-symbols-outlined !text-lg text-[var(--accent)] drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]">{t("ui_icon_troubleshoot") || "troubleshoot"}</span>
                      {t("settings_auto_detect") || "AUTO-DETECT"}
                    </button>
                  }
               >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full">
      ...
      <div className="group">
        ...
        />
               ...
               >
                 <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
                  <div className="flex flex-col gap-8">
                    <div className="group">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_engine_agency") || "Defcon Agency Level"}</label>
                      <CustomSettingsDropdown 
                        value={config.engine_agency_level || 0} 
                        onChange={(val: any) => updateConfig('engine_agency_level', val)}
                        options={[
                          { id: 0, label: t("settings_agency_none") || "None (Passive Monitoring)" },
                          { id: 1, label: t("settings_agency_basic") || "Basic (Alerts Only)" },
                          { id: 2, label: t("settings_agency_adv") || "Advanced (Auto-Intercept)" }
                        ]} 
                      />
                    </div>
                    <div className="group">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_engine_retention") || "Engine Core Retention Policy"}</label>
                      <CustomSettingsDropdown 
                        ...
                        options={[
                          ...
                          { id: 5, label: t("settings_keep_5") || "Keep Last 5 (Default)" },
                          { id: 10, label: t("settings_keep_10") || "Keep Last 10" },
                          { id: 999, label: t("settings_keep_all") || "Never Auto-Purge" }
                        ]} 
                      />
                    </div>
                    <div className="group">
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="group">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_defcon_target") || "Defcon Backup Target"}</label>
                      <CustomSettingsDropdown 
                        value={config.defcon_backup_target || 0} 
                        onChange={(val: any) => updateConfig('defcon_backup_target', val)}
                        options={[
                          { id: 0, label: t("settings_target_both") || "Both (Recommended)" },
                          { id: 1, label: t("settings_target_world") || "World State Only" },
                          { id: 2, label: t("settings_target_engine") || "Engine Core Only" }
                        ]} 
                      />
                    </div>
                    <div className="group">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] mb-3 ml-4 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_world_retention") || "World State Retention Policy"}</label>
                      <CustomSettingsDropdown 
           ...
           {activeTab === 'AUTH' && (
             <TabContainer title={t("settings_auth_title") || "Network Credentials"} icon={TABS.find(t=>t.id==='AUTH')?.icon}>
                <div className="grid xl:grid-cols-2 gap-12">
                 <div className="flex flex-col gap-8">
                   {session?.user?.id && (
                     <div className="flex flex-col gap-3 group">
                       <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-2 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] transition-colors">{t("settings_auth_user_id") || "System ID"}</label>
                   ...
                   <div className="flex flex-col gap-3 group">
                     <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-2 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] transition-colors">{t("settings_auth_email") || "Update Comms (Email)"}</label>
                     <div className="flex gap-2">
                       <input 
                         type="email" 
                         value={emailInput}
                       ...
                       >
                         {t("settings_btn_update") || "Update Credentials"}
                       </button>
                     </div>
                   </div>

                   <div className="flex flex-col gap-3 group">
                     <label className="text-[10px] font-black uppercase tracking-[0.2em] ml-2 text-[var(--subtext)] opacity-70 group-hover:text-[var(--text)] transition-colors">{t("settings_auth_password") || "Update Passcode (Password)"}</label>
                     <div className="flex gap-2">
                       <input 
                         type="password" 
                         value={passwordInput}
           ...
           {activeTab === 'NOTIFICATIONS' && (
             <TabContainer 
                title={t("settings_tab_notifs") || "Notifications"}
                icon={TABS.find(t=>t.id==='NOTIFICATIONS')?.icon}
                actions={
                  <button onClick={() => setShowMasonPanel(true)} className={standardButtonClass}>
                    <span className="material-symbols-outlined lowercase theme-text-accent text-lg">{t("emote_bell") || "notifications"}</span> {t("settings_notif_per_mason") || "Per-Mason Broadcast Alerts"}
                  </button>
                }
             >
               <div className="grid xl:grid-cols-2 gap-8">
                 <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={toggleNotifyReplies}>
                   <div className="flex flex-col gap-2">
                     <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_notify_replies") || "Replies"}</span>
                     <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_notify_replies_desc") || "Notify when someone replies to your transmissions."}</span>
                   </div>
                 ...
                 <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={toggleNotifySupport}>
                   <div className="flex flex-col gap-2">
                     <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_notify_support") || "SUPPORT REPLIES"}</span>
                     <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_notify_support_desc") || "Notify when you receive a reply on a support ticket."}</span>
                   </div>
                 ...
                 <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={toggleNotifyNewPosts}>
                   <div className="flex flex-col gap-2">
                     <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_notify_new_posts") || "New Transmissions"}</span>
                     <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_notify_new_posts_desc") || "Notify when followed Masons broadcast new transmissions."}</span>
                   </div>
                 ...
                 <div className="flex items-center justify-between p-8 rounded-3xl theme-glass-panel hover:theme-border-accent transition-all cursor-pointer group shadow-xl hover:scale-[1.02] active:scale-95" onClick={toggleNotifyAuthorOnly}>
                   <div className="flex flex-col gap-2">
                     <span className="text-sm font-black uppercase tracking-[0.2em] text-[var(--text)] group-hover:theme-text-accent transition-colors">{t("settings_notify_author_only") || "Author-Only Replies"}</span>
                     <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed">{t("settings_notify_author_only_desc") || "Only notify on replies from the original transmission author."}</span>
                   </div>
           ...
           {activeTab === 'CHAMELEON' && (
             <TabContainer 
               title={t("settings_chameleon_title") || "Chameleon Library"}
               icon={TABS.find(t=>t.id==='CHAMELEON')?.icon}
               actions={
                 <>
                   <button onClick={() => { setMarketTab('CHAMELEONS'); setView('marketplace'); }} className={standardButtonClass}>{t("market_btn_browse") || "Browse"}</button>
                   <button onClick={handleImportTheme} className={standardButtonClass}>{t("settings_btn_import") || "Import"}</button>
                   <button onClick={createNewTheme} className={standardButtonClass}>{t("settings_btn_create") || "CREATE"}</button>
                 </>
               }
             >
                 ...
                 {Object.entries({ ...CORE_THEMES, ...customThemes }).map(([id, data]: any) => (
                   <button key={id} onClick={() => setActiveThemeId(id)} className={`p-6 rounded-3xl border transition-all text-left group relative backdrop-blur-xl shadow-lg ${activeThemeId === id ? 'theme-border-accent theme-glass-inner shadow-[0_0_30px_rgba(var(--accent-rgb),0.2)]' : 'theme-glass-panel opacity-80 hover:opacity-100'}`}>
                     ...
                     <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                       <div onClick={(e) => handleExportTheme(e, data)} className="p-2 rounded-full hover:bg-white/10 text-sm material-symbols-outlined lowercase">{t("ui_icon_save") || "save"}</div>
                       {!CORE_THEMES[id] && (
                         <>
                           <div onClick={(e) => { e.stopPropagation(); setNewThemeName(data.name); setEditingThemeId(id); }} className="p-2 rounded-full hover:bg-white/10 text-sm theme-text-accent material-symbols-outlined lowercase">{t("ui_icon_edit") || "edit"}</div>
                           <div onClick={(e) => { e.stopPropagation(); deleteTheme(id); }} className="p-2 rounded-full hover:bg-white/10 text-sm theme-text-danger material-symbols-outlined lowercase">{t("ui_icon_delete") || "delete"}</div>
                         </>
                       )}
                     </div>
                   </button>
               ...
               <div className="mt-8 pt-10 border-t border-white/5">
                  <h3 className="text-xl font-black uppercase tracking-widest text-[var(--text)] mb-8 flex items-center gap-3">
                     <span className="material-symbols-outlined text-2xl theme-text-accent">{t("ui_icon_tune") || "tune"}</span>
                     {t("settings_forge_live") || "Chameleon Theme Forge"}
                  </h3>
                              ...
                              <div className="fixed z-[50001] p-8 theme-glass-panel rounded-[2rem] border border-[color-mix(in_srgb,var(--text)_10%,transparent)] shadow-[0_0_50px_rgba(0,0,0,0.8)] w-[26rem] animate-in fade-in zoom-in-95 duration-200"
                                ...
                                <div className="flex gap-4 mb-8">
                                  ...
                                  <div className="w-12 h-12 rounded-2xl border border-white/10 shrink-0 shadow-inner flex items-center justify-center cursor-pointer hover:scale-110 transition-transform group/fav" style={{ backgroundColor: currentTheme[key] }} onClick={() => toggleFavColor(currentTheme[key])}>
                                     {favColors.includes(currentTheme[key]) ? <span className="text-yellow-500 text-2xl drop-shadow-md">{t("emote_star")}</span> : <span className="opacity-0 group-hover/fav:opacity-50 text-white font-black text-2xl material-symbols-outlined">{t("ui_icon_plus") || "add"}</span>}
                                  </div>
                                </div>
                                ...
                                {favColors.length > 0 && (
                                  <div className="mb-8 pt-6 border-t border-white/10">
                                     <h4 className="text-[10px] font-black uppercase tracking-widest text-[var(--subtext)] mb-4">{t("settings_color_favs") || "Saved Colors"}</h4>
                                ...
                                <div className="flex flex-col gap-4 pt-6 border-t border-white/10">
                                  <div className="flex items-center gap-4">
                                    <span className="text-xs font-black opacity-50 w-3 text-center text-red-500">{t("settings_color_r") || "R"}</span>
                                    <input 
                                  ...
                                  <div className="flex items-center gap-4">
                                    <span className="text-xs font-black opacity-50 w-3 text-center text-green-500">{t("settings_color_g") || "G"}</span>
                                    <input 
                                      type="range" min="0" max="255" 
                                  ...
                                  <div className="flex items-center gap-4">
                                    <span className="text-xs font-black opacity-50 w-3 text-center text-blue-500">{t("settings_color_b") || "B"}</span>
                                    <input 
                                      type="range" min="0" max="255" 
           ...
           {activeTab === 'LEXICON' && (
             <TabContainer 
               title={t("settings_lexicon_title") || "Lexicon Library"}
               icon={TABS.find(t=>t.id==='LEXICON')?.icon}
               actions={
                 <>
                   <button onClick={() => { setMarketTab('LEXICONS'); setView('marketplace'); }} className={standardButtonClass}>{t("market_btn_browse") || "Browse"}</button>
                   <button onClick={handleImportLexicon} className={standardButtonClass}>{t("settings_btn_import") || "Import"}</button>
                 </>
               }
             >
               <div className="flex flex-col gap-12">
                 ...
                 <div className="flex flex-col gap-6">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--subtext)] opacity-60 ml-2">{t("settings_installed_lexicons") || "Active System Lexicon"}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      {favoriteLexicons.map(code => (
                        <button key={code} onClick={() => setActiveLang(code)} className={`p-6 rounded-3xl border transition-all text-left flex justify-between items-center group relative backdrop-blur-xl shadow-lg ${activeLang === code ? 'theme-border-accent theme-glass-inner shadow-[0_0_30px_rgba(var(--accent-rgb),0.2)]' : 'theme-glass-panel opacity-80 hover:opacity-100'}`}>
                           ...
                           <div className="flex items-center gap-3 shrink-0">
                             <span className={`text-2xl transition-all material-symbols-outlined ${activeLang === code ? 'opacity-100 theme-text-accent' : 'opacity-0 scale-50 w-0'}`}>{t("ui_icon_check") || "check"}</span>
                 ...
                 <div className="flex flex-col gap-6 mt-8 pt-10 border-t border-white/5">
                    <h3 className="text-xl font-black uppercase tracking-widest text-[var(--text)] mb-2 flex items-center gap-3">
                       <span className="material-symbols-outlined text-2xl theme-text-accent">{t("ui_icon_menu_book") || "menu_book"}</span>
                       {t("settings_library") || "Available Lexicons"}
                    </h3>
                    ...
                    <div className="relative mt-2">
                       ...
                       />
                       <span className="absolute right-6 top-1/2 -translate-y-1/2 opacity-50 text-xl material-symbols-outlined">{t("ui_icon_search") || "search"}</span>
                    </div>
                         ...
                         .map(code => (
                           <div key={code} className={`p-6 rounded-3xl border transition-all flex flex-col gap-2 relative group cursor-pointer backdrop-blur-xl shadow-lg ${activeLang === code ? 'theme-border-accent theme-glass-inner shadow-[0_0_30px_rgba(var(--accent-rgb),0.2)]' : 'theme-glass-panel opacity-80 hover:opacity-100'}`} onClick={() => setActiveLang(code)}>
                              ...
                              <div className="absolute top-1/2 right-6 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                                 {activeLang === code && <span className="theme-text-accent text-3xl material-symbols-outlined">{t("ui_icon_check") || "check"}</span>}
                              </div>
                              {code !== 'en-sanctuary' && code !== 'en-default' && code !== 'de-default' && (
                                <div className="absolute bottom-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                   <button onClick={(e) => { e.stopPropagation(); deleteLexicon(code); }} className="p-3 rounded-full hover:bg-red-500/20 text-sm theme-text-danger shadow-sm transition-colors flex items-center justify-center">
                                      <span className="material-symbols-outlined !text-lg">{t("ui_icon_trash") || "delete"}</span>
                                   </button>
                                </div>
                              )}
                           </div>
           ...
           {activeTab === 'LOGIC' && (
             <TabContainer title={t("settings_anarchy_title") || "Logic Integrity (Anarchy Protocols)"} icon={TABS.find(t=>t.id==='LOGIC')?.icon}>
               <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                  <div onClick={() => toggleRule('highlander')} className={`p-8 rounded-3xl border transition-all cursor-pointer flex items-center gap-8 group backdrop-blur-xl shadow-xl ${!rules.highlander ? 'theme-glass-inner border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'theme-glass-inner hover:border-white/20'}`}>
                    ...
                    <div className="flex flex-col pr-4">
                      <span className={`text-sm font-black uppercase tracking-widest transition-colors ${!rules.highlander ? 'text-red-400' : 'text-[var(--text)] group-hover:text-white'}`}>{t("settings_anarchy_highlander") || "Highlander Enforcement (Flavors)"}</span>
                      <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed mt-2">{t("settings_anarchy_highlander_desc") || "ON: Auto-evict rivals. OFF: The More the Merrier (Gladiator Mode)."}</span>
                    </div>
                  </div>
                  ...
                  <div onClick={() => toggleRule('family')} className={`p-8 rounded-3xl border transition-all cursor-pointer flex items-center gap-8 group backdrop-blur-xl shadow-xl ${!rules.family ? 'theme-glass-inner border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'theme-glass-inner hover:border-white/20'}`}>
                    ...
                    <div className="flex flex-col pr-4">
                      <span className={`text-sm font-black uppercase tracking-widest transition-colors ${!rules.family ? 'text-red-400' : 'text-[var(--text)] group-hover:text-white'}`}>{t("settings_anarchy_family") || "Nuclear Family Cohesion (Twins)"}</span>
                      <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed mt-2">{t("settings_anarchy_family_desc") || "ON: Auto-selects siblings. OFF: Single Parent Mode."}</span>
                    </div>
                  </div>
                  ...
                  <div onClick={() => toggleRule('dependencies')} className={`p-8 rounded-3xl border transition-all cursor-pointer flex items-center gap-8 group backdrop-blur-xl shadow-xl ${!rules.dependencies ? 'theme-glass-inner border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'theme-glass-inner hover:border-white/20'}`}>
                    ...
                    <div className="flex flex-col pr-4">
                      <span className={`text-sm font-black uppercase tracking-widest transition-colors ${!rules.dependencies ? 'text-red-400' : 'text-[var(--text)] group-hover:text-white'}`}>{t("settings_anarchy_deps") || "Dependency Chain-Link"}</span>
                      <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed mt-2">{t("settings_anarchy_deps_desc") || "ON: Pulls in required libraries. OFF: Lone Wolf (Script Call Failed)."}</span>
                    </div>
                  </div>
                  ...
                  <div onClick={() => toggleRule('intercept')} className={`p-8 rounded-3xl border transition-all cursor-pointer flex items-center gap-8 group backdrop-blur-xl shadow-xl ${!rules.intercept ? 'theme-glass-inner border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'theme-glass-inner hover:border-white/20'}`}>
                    ...
                    <div className="flex flex-col pr-4">
                      <span className={`text-sm font-black uppercase tracking-widest transition-colors ${!rules.intercept ? 'text-red-400' : 'text-[var(--text)] group-hover:text-white'}`}>{t("settings_anarchy_intercept") || "Conflict Intercept (Collision Severity 4)"}</span>
                      <span className="text-[10px] font-bold text-[var(--subtext)] opacity-60 uppercase tracking-widest leading-relaxed mt-2">{t("settings_anarchy_intercept_desc") || "ON: Block mortal enemies. OFF: Let them fight."}</span>
                    </div>
                  </div>
               </div>
             </TabContainer>
           ...
           {activeTab === 'MALWARE' && (
             <TabContainer title={t("settings_tab_malware") || "Malware Protocol"} icon={TABS.find(t=>t.id==='MALWARE')?.icon}>
               <div className="flex flex-col items-center justify-center p-16 text-center border border-red-500/20 rounded-[3rem] bg-red-950/20 backdrop-blur-xl shadow-[0_0_50px_rgba(239,68,68,0.1)] mt-8">
                 ...
                 <button onClick={() => { setShowMalwareOverride(true); setMalwareToggleActive(false); setOverrideInput(''); }} className="w-full max-w-lg py-8 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] transition-all shadow-xl border bg-black/40 text-red-500 border-red-500/30 hover:bg-red-900/40 hover:border-red-500 hover:shadow-[0_0_40px_rgba(239,68,68,0.3)]">
                   {t("settings_malware_btn") || "Configure Malware Protocol"}
                 </button>
               </div>
             </TabContainer>
           )}
      ...
      <SidePanel 
        ...
        subtitle={t("settings_notif_per_mason_desc") || "Enable specific transmission alerts for Masons you follow."}
      ...
      >
        <div className="flex flex-col space-y-4 relative z-10 w-full h-full p-4">
          {followedMasons.length === 0 && <div className="text-[10px] font-black uppercase tracking-widest text-[var(--subtext)] opacity-50 text-center mt-12 p-8 theme-glass-inner border border-dashed border-white/10 rounded-3xl">{t("settings_no_masons") || "No Masons Followed"}</div>}
      ...
      <SidePanel
        ...
        title={t("settings_malware_title") || "Malware Protocol"}
        icon="skull"
        ...
        <div className="flex-1 flex flex-col gap-8 relative">
          ...
          <div className="flex flex-col gap-4 min-w-0 flex-1 relative z-10 text-center mb-8">
            ...
            <p className="text-xs font-black text-red-400 opacity-80 uppercase tracking-widest leading-relaxed">
              {t("settings_malware_subtitle") || "Network Threat Defense Systems"}
            </p>
          </div>
          ...
          <div className="relative z-10 flex items-center justify-between p-8 rounded-[2rem] theme-glass-inner backdrop-blur-2xl border border-red-500/20 shadow-2xl cursor-pointer hover:border-red-500/50 hover:scale-[1.02] transition-all group mt-4" onClick={() => setMalwareToggleActive(!malwareToggleActive)}>
            <div className="flex flex-col gap-2 text-left">
              <span className="text-sm font-black uppercase tracking-[0.2em] text-red-400 group-hover:text-red-300 transition-colors">{t("settings_malware_disable") || "Disable Quarantine Bay"}</span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-red-200/50">{t("settings_malware_disable_desc") || "Allow execution of malicious signatures"}</span>
            </div>
            ...
            <div className="flex flex-col gap-8 animate-in slide-in-from-top-4 fade-in duration-300 relative z-10 mt-4">
              <div className="p-8 bg-red-950/30 border border-red-500/30 rounded-[2rem] text-center shadow-inner">
                <p className="text-[10px] font-black text-red-400 uppercase tracking-[0.2em] leading-relaxed">
                  {t("settings_malware_warning") || "WARNING: By bypassing the Quarantine Bay, you are willingly exposing the global network to known malicious DNA signatures. If you proceed, your Supabase JWT will be revoked and your hardware ID will be permanently blacklisted from Sanctuary OS global databases."}
                </p>
              </div>
              <div className="flex flex-col gap-4">
                <label className="text-[10px] font-black text-red-500 uppercase tracking-widest text-center">{t("settings_malware_type_confirm") || "Type \"I Confirm\" to acknowledge the risks"}</label>
                <input value={overrideInput} onChange={(e) => setOverrideInput(e.target.value)} className="w-full bg-red-950/20 backdrop-blur-md theme-glass-panel border border-red-500/50 rounded-2xl py-6 text-center text-red-300 font-black text-xl uppercase tracking-widest focus:outline-none focus:bg-red-950/40 focus:border-red-400 focus:shadow-[0_0_30px_rgba(239,68,68,0.4)] shadow-inner transition-all" placeholder={t("settings_malware_confirm_text") || "I Confirm"} />
              </div>
              
              <div className="flex gap-4 mt-4 mb-8">
                <button disabled={overrideInput.toLowerCase() !== (t("settings_malware_confirm_text") || "I Confirm").toLowerCase()} onClick={executeOverride} className="flex-1 py-5 bg-red-600/20 theme-glass-panel border border-red-500/50 hover:bg-red-600/40 text-red-100 hover:text-white backdrop-blur-xl shadow-[0_0_30px_rgba(220,38,38,0.4)] rounded-xl font-black text-[10px] uppercase tracking-[0.3em] disabled:opacity-30 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-95 transition-all">{t("settings_malware_sever") || "Sever Access"}</button>
                <button onClick={() => { setShowMalwareOverride(false); setMalwareToggleActive(false); setOverrideInput(''); }} className="flex-1 py-5 theme-glass-panel border border-white/10 hover:border-white/30 backdrop-blur-lg rounded-xl font-black text-[10px] uppercase text-red-200 tracking-widest transition-all hover:bg-red-950/40 shadow-lg">{t("radar_tier3_cancel") || "CANCEL"}</button>
              </div>
            </div>
          ) : (
