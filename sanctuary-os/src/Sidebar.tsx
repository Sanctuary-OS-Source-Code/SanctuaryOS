  ...
  return (
    <div className="px-4 relative group/navbtn">
      ...
      >
        ...
        <div className="absolute inset-0 rounded-[inherit] bg-gradient-to-r from-transparent via-[color-mix(in_srgb,var(--text)_10%,transparent)] to-transparent -translate-x-[150%] group-hover:translate-x-[150%] transition-transform duration-1000 ease-in-out pointer-events-none" />
...
export function Sidebar({
  ...
  return (
    <nav
      className={`${isSidebarCollapsed ? 'w-[80px]' : ''} flex-shrink-0 h-full flex flex-col relative z-20 transition-all duration-500 rounded-3xl border border-[color-mix(in_srgb,var(--sidebartext)_10%,transparent)] ${!isConfigured ? 'opacity-50 cursor-not-allowed' : ''}`}
      style={{
        width: isSidebarCollapsed ? '80px' : 'var(--sidebarWidth, 288px)',
        minWidth: isSidebarCollapsed ? '80px' : 'var(--sidebarWidth, 288px)',
        boxShadow: `10px 0 30px -10px color-mix(in srgb, var(--bg) 80%, transparent)`
      }}
    >
      ...
      />
      <div className={`flex flex-col h-full w-full ${!isConfigured ? 'pointer-events-none' : ''}`}>
          ...
          <div className="p-4 pb-4 relative z-30">
            ...
            <div className="relative group/nav mt-2">
              <button
                ...
                className={`relative w-full py-4 rounded-2xl bg-transparent text-[10px] font-black capitalize tracking-[0.2em] transition-all duration-300 flex items-center justify-center gap-3 shadow-lg group/btn overflow-hidden border ${!isConfigured
      ...
      {isSidebarCollapsed && hoveredTooltip && createPortal(
        ...
        >
          <div
            className="flex flex-col items-start justify-center px-5 py-3 max-w-[320px] w-max shadow-2xl border border-[color-mix(in_srgb,var(--text)_10%,transparent)] rounded-xl overflow-hidden bg-[color-mix(in_srgb,var(--text)_5%,transparent)] backdrop-blur-md animate-in fade-in slide-in-from-left-2"
          >
