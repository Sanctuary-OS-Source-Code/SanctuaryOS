  ...
  return (
    <div className={`flex flex-col h-full relative items-start animate-in fade-in zoom-in-95 duration-500 ${hasScanned || loading || error ? 'pb-24' : ''}`}>
        ...
        <div className="relative z-10 flex flex-col gap-8 w-full h-full">
          <ViewHeader title={t("radar_title")} subtitle={t("radar_subtitle")} icon={t("icon_track_changes")} iconColorClass="text-amber-400 border-amber-500/30">
      ...
      {stats.packages > 0 && !error && (
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="theme-glass-panel p-8 rounded-[var(--radius)] border border-[color-mix(in_srgb,var(--text)_10%,transparent)] shadow-lg relative overflow-hidden flex flex-col justify-center group transition-all hover:border-[color-mix(in_srgb,var(--text)_20%,transparent)] hover:-translate-y-1">
                <div className="absolute right-[-20px] top-1/2 -translate-y-1/2 opacity-[0.03] group-hover:opacity-[0.06] group-hover:scale-110 transition-all duration-700 pointer-events-none">
                  <span className="material-symbols-outlined !text-[160px]">{t("icon_inventory_2")}</span>
                </div>
              ...
              <div className="flex items-center justify-between border-b theme-border-danger pb-4 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <h2 className="text-2xl font-black theme-text-danger uppercase tracking-tighter italic drop-shadow-md">{t("tier4_title") || "Collision Severity 4"?.replace("dY>` ", "") || "FATAL ENGINE CLASHES"}</h2>
              ...
              {confirmMassVault && (
                <div className="animate-in slide-in-from-top-2 p-6 theme-glass-panel border-white/10 rounded-[var(--radius)] flex flex-col md:flex-row gap-6 items-center justify-between shadow-xl mb-6">
                  <p className="text-sm font-black theme-text-danger uppercase tracking-widest">
                    {t("secure_quarantine") || "SECURE {count} TARGETS IN QUARANTINE".replace("{count}", String(selectedForVault.length)) || `Yeet ${selectedForVault.length} duplicates to the Vault?`}
                  </p>
          ...
          {softConflicts.length > 0 && (
            <details className="group space-y-6 theme-glass-inner p-6 rounded-[var(--radius)] border border-white/5 cursor-pointer mb-32">
      ...
      {hasScanned && stats.packages === 0 && !loading && !error && (
        <div className="py-24 flex flex-col items-center justify-center text-center space-y-6 opacity-60 animate-in fade-in zoom-in-95 duration-500">
          <div className="w-24 h-24 rounded-full theme-panel-accent border flex items-center justify-center">
            <span className="material-symbols-outlined !text-[48px] grayscale">{t("icon_folder")}</span>
          </div>
          <div>
      ...
      {error && (
        <div className="theme-panel-danger border p-8 rounded-[var(--radius)] flex items-start gap-4">
          <span className="material-symbols-outlined !text-[32px]">{t("icon_warning_amber")}</span>
          <div>
            <h3 className="theme-text-danger font-black uppercase tracking-widest mb-1 text-sm">{t("radar_malfunction")}</h3>
      ...
      >
        <div className="flex flex-col gap-6">
           <div className="theme-glass-panel border border-white/5 rounded-[var(--radius)] p-6 shadow-lg relative group/card shrink-0 h-[240px]">
             <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-500 pointer-events-none rounded-[var(--radius)]" />
             <div className="flex items-center gap-4 mb-6 relative z-10">
               <div className="w-10 h-10 rounded-[0.85rem] bg-black/20 flex items-center justify-center border border-white/10 shadow-inner text-amber-400">
                 <span className="material-symbols-outlined !text-[20px]">{t("icon_my_location")}</span>
               </div>
               <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[var(--text)] drop-shadow-md">{t("sidebar_scope")}</h3>
             </div>
             ...
             <div className="relative z-10 flex flex-col gap-4">
               ...
               />
               <p className="text-[10px] font-bold text-[var(--subtext)] opacity-60 text-center px-4 leading-relaxed tracking-wide">{t("sidebar_scope_desc")}</p>
             </div>
           </div>
