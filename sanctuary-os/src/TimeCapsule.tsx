...
export default function TimeCapsule({
  ...
  const vaultCapacityGb = config?.vault_capacity_gb || 0; // 0 means unlimited
  ...
  const renderBackupCard = (backup: any) => {
    ...
    const title = sig?.alias || (isEngine ? t("backups_engine_full") || "ENGINE CORE" : t("backups_world_state") || "WORLD STATE");
    const icon = isEngine ? t("ui_icon_engine") || "settings" : t("ui_icon_globe") || "public";
    ...
    return (
      <div key={backupName} className={`relative bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 p-5 rounded-3xl flex flex-col gap-4 shadow-xl min-h-[13rem] overflow-hidden group ${themeHoverBorder} transition-all duration-300`}>
        {/* Background Gradient */}
        ...
        <div className="flex justify-between items-start relative z-10">
          ...
          {sizeMb > 0 && (
             <span className="text-[9px] font-black text-[var(--subtext)] uppercase tracking-widest bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 px-2.5 py-1 rounded-lg">{(sizeMb / 1024).toFixed(2)} {t("unit_gb") || "GB"}</span>
          )}
        </div>
        ...
        <div className="flex flex-col relative z-10 mt-1">
          ...
          <div className="flex items-center gap-2">
             <div className={`w-2 h-2 rounded-full ${themeLed} animate-pulse`} />
        ...
        <div className="mt-auto relative z-20 h-10">
           ...
           <div className={`absolute inset-0 flex gap-2 transition-all duration-300 ${confirmRestoreBackup === backupName || confirmDeleteBackup === backupName ? 'opacity-0 translate-y-4 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
              ...
              >
                 <span className={`material-symbols-outlined !text-sm`}>{t("ui_icon_restore") || "restore"}</span>
                 {t("backups_btn_restore") || "RESTORE"}
              </button>
              <button 
              ...
              >
                <span className="material-symbols-outlined !text-sm group-hover/del:scale-110 transition-transform">{t("ui_icon_trash") || "delete"}</span>
              </button>
           </div>
           ...
           <div className={`absolute inset-0 flex gap-2 transition-all duration-300 ${confirmRestoreBackup === backupName ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
              ...
              >
                 {isEngine ? t("confirm_restore_engine_card") || "RESTORE THIS ENGINE?" : t("confirm_restore_state") || "RESTORE THIS STATE?"}
              </button>
           ...
           <div className={`absolute inset-0 flex gap-2 transition-all duration-300 ${confirmDeleteBackup === backupName ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
              ...
              >
                 {isEngine ? t("confirm_delete_engine_card") || "DELETE PERMANENTLY THIS ENGINE?" : t("confirm_delete_state") || "DELETE PERMANENTLY THIS STATE?"}
              </button>
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-700 pb-12 w-full">
      <ViewHeader title={t("backups_title") || "Time Capsule"} subtitle={t("backups_subtitle") || "Backups, restores, and versioned world states"} icon={t("ui_icon_history") || "history"} iconColorClass="text-[var(--accent)] border-[var(--accent)]/30">
      ...
      {/* STORAGE METRICS DASHBOARD - NEW LAYOUT */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 w-full mb-4">
        
        {/* World State Space Card */}
        <div className="bg-black/5 dark:bg-white/5 p-6 rounded-3xl border border-black/5 dark:border-white/5 shadow-md flex flex-col gap-6 relative overflow-hidden group hover:border-indigo-500/30 transition-all duration-500">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-500 shadow-sm shrink-0">
              <span className="material-symbols-outlined !text-[24px]">{t("ui_icon_globe") || "public"}</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-500 opacity-80 mb-1">{t("backups_world_space") || "WORLD STATE SPACE"}</span>
              <span className="text-2xl font-black text-[var(--text)] tracking-tighter">{(totalWorldSize / 1024).toFixed(2)} <span className="text-sm text-[var(--subtext)]">{t("unit_gb") || "GB"}</span></span>
            </div>
          </div>
          
          <div className="flex flex-col gap-2 w-full mt-2 relative z-10">
            <div className="flex justify-end items-center w-full">
              <span className="text-[10px] font-black text-indigo-500 tracking-widest">{worldPercentage.toFixed(1)}%</span>
            </div>
        ...
        {/* Engine Core Space Card */}
        <div className="bg-black/5 dark:bg-white/5 p-6 rounded-3xl border border-black/5 dark:border-white/5 shadow-md flex flex-col gap-6 relative overflow-hidden group hover:border-rose-500/30 transition-all duration-500">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="w-12 h-12 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-rose-500 shadow-sm shrink-0">
              <span className="material-symbols-outlined !text-[24px]">{t("ui_icon_engine") || "settings"}</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-rose-500 opacity-80 mb-1">{t("backups_engine_space") || "ENGINE CORE SPACE"}</span>
              <span className="text-2xl font-black text-[var(--text)] tracking-tighter">{(totalEngineSize / 1024).toFixed(2)} <span className="text-sm text-[var(--subtext)]">{t("unit_gb") || "GB"}</span></span>
            </div>
          </div>
          
          <div className="flex flex-col gap-2 w-full mt-2 relative z-10">
            <div className="flex justify-end items-center w-full">
              <span className="text-[10px] font-black text-rose-500 tracking-widest">{enginePercentage.toFixed(1)}%</span>
            </div>
            <div className="h-1.5 w-full bg-black/10 dark:bg-black/40 rounded-full overflow-hidden flex shadow-inner border border-black/5 dark:border-white/5">
        ...
        {/* Total Space Card */}
        <div className="bg-black/5 dark:bg-white/5 p-6 rounded-3xl border border-black/5 dark:border-white/5 shadow-md flex flex-col gap-6 relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-500">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-500 shadow-sm shrink-0">
              <span className="material-symbols-outlined !text-[24px]">{t("ui_icon_storage") || "storage"}</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500 opacity-80 mb-1">{t("backups_total_space") || "TOTAL VAULT SPACE USED"}</span>
              <span className="text-2xl font-black text-[var(--text)] tracking-tighter">
                ...
                <span className="text-sm text-[var(--subtext)] ml-1">
                  {vaultCapacityGb > 0 ? `/ ${vaultCapacityGb}` : ''} {t("unit_gb") || "GB"}
                </span>
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 w-full mt-2 relative z-10">
            <div className="flex justify-end items-center w-full">
              <span className="text-[10px] font-black text-emerald-500 tracking-widest">{(worldPercentage + enginePercentage).toFixed(1)}%</span>
            </div>
            {/* Unified tiny progress bar */}
            <div className="h-1.5 w-full bg-black/10 dark:bg-black/40 rounded-full overflow-hidden flex shadow-inner border border-black/5 dark:border-white/5">
      ...
      <div className="flex flex-col gap-10 pt-4">
        {backupList?.length > 0 ? (
              ...
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h3 className="text-sm font-black text-[var(--text)] uppercase tracking-[0.2em] flex items-center gap-4 border-b border-black/5 dark:border-white/5 pb-4">
                  <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center shadow-sm">
                    <span className="text-indigo-500 text-[20px] material-symbols-outlined">{t("ui_icon_globe") || "public"}</span>
                  </div>
                  {t("backups_section_world") || "WORLD STATES"}
                </h3>
              ...
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
                <h3 className="text-sm font-black text-[var(--text)] uppercase tracking-[0.2em] flex items-center gap-4 border-b border-black/5 dark:border-white/5 pb-4 mt-4">
                  <div className="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center shadow-sm">
                    <span className="text-rose-500 text-[20px] material-symbols-outlined">{t("ui_icon_engine") || "settings"}</span>
                  </div>
                  {t("backups_section_engine") || "ENGINE CORES"}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-center h-64 theme-glass-panel border border-white/5 rounded-[3rem] shadow-xl">
            <span className="text-[var(--subtext)] font-black uppercase tracking-widest opacity-60 flex items-center gap-4">
              <span className="material-symbols-outlined !text-3xl opacity-50">{t("ui_icon_hourglass_empty") || "hourglass_empty"}</span>
              {t("timecapsule_no_backups") || "No Archives Found"}
            </span>
          </div>
        )}
      </div>
      ...
      <SidePanel
          ...
          title={t("backups_tools_title") || "CAPSULE OPERATIONS"}
          ...
          <div className="flex flex-col gap-6">
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 shadow-lg relative overflow-hidden group/card">
               ...
               <div className="flex items-center gap-4 mb-6 relative z-10">
                 <div className="w-10 h-10 rounded-[0.85rem] bg-black/20 flex items-center justify-center border border-white/10 shadow-inner text-emerald-500">
                   <span className="material-symbols-outlined !text-[20px]">{t("ui_icon_verified") || "verified_user"}</span>
                 </div>
               ...
               <div className="relative z-10">
                 <div className="bg-[color-mix(in_srgb,var(--text)_5%,transparent)] border border-white/10 rounded-2xl px-6 py-4 flex items-center justify-between shadow-inner">
                   <span className="text-sm font-black tracking-widest text-[var(--text)] uppercase flex items-center gap-3">
                     <span className="material-symbols-outlined !text-[18px] text-emerald-500">{t("ui_icon_verified") || "verified_user"}</span>
             ...
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 shadow-lg relative overflow-hidden group/card">
               ...
               <div className="flex items-center gap-4 mb-6 relative z-10">
                 ...
                 <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[var(--text)] drop-shadow-md">{t("backups_sidebar_actions") || "ACTIONS"}</h3>
               </div>
                  ...
                  {!confirmSealWorld ? (
                    ...
                    <div className="flex gap-2 h-16">
                      <button onClick={() => { triggerPrePatchSnapshot && triggerPrePatchSnapshot(true); setIsSidePanelOpen(false); setConfirmSealWorld(false); }} className="flex-[2] h-full rounded-2xl bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-500 border border-indigo-500/30 font-black text-[10px] tracking-widest flex items-center justify-center gap-2">
                         <span className="material-symbols-outlined !text-sm">{t("ui_icon_check_circle") || "check_circle"}</span> {t("ui_btn_confirm") || "CONFIRM"}
                      </button>
                  ...
                  {!confirmSealEngine ? (
                    ...
                    <div className="flex gap-2 h-16">
                      <button onClick={() => { triggerFullEngineBackup && triggerFullEngineBackup(); setIsSidePanelOpen(false); setConfirmSealEngine(false); }} className="flex-[2] h-full rounded-2xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/30 font-black text-[10px] tracking-widest flex items-center justify-center gap-2">
                         <span className="material-symbols-outlined !text-sm">{t("ui_icon_warning") || "warning_amber"}</span> {t("ui_btn_confirm") || "CONFIRM"}
                      </button>
