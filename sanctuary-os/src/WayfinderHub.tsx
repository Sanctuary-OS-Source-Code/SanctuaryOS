...
export default function WayfinderHub({ onOpenMasonProfile }: { onOpenMasonProfile?: (id: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full pb-48 relative">
      <ViewHeader title={t("wf_hub_title")} subtitle={t("wf_hub_subtitle")} icon={t("ui_icon_terminal")} iconColorClass="text-indigo-400 border-indigo-500/30">
        <div className="flex items-center theme-glass-panel rounded-2xl p-1 border border-white/10 shadow-inner">
           ...
           >
             <span className="material-symbols-outlined !text-[16px] group-hover:scale-110 transition-transform">{t("ui_icon_verified")}</span>
             {t("wf_hub_verify")}
           </button>
      ...
      <div className="flex flex-col gap-1 w-full mb-4 shrink-0">
        <div className="flex items-center gap-1 overflow-x-auto custom-scrollbar p-1 theme-glass-panel rounded-2xl border border-white/5 shadow-inner">
           <HubTabButton id="command_center" icon={t("ui_icon_pc")} label={t("wf_tab_command")} activeTab={activeTab} setTab={setActiveTab} />
           <HubTabButton id="wayfinder_comms" icon={t("ui_icon_satellite_alt")} label={t("wf_tab_dispatch")} activeTab={activeTab} setTab={setActiveTab} />
           ...
           <HubTabButton id="reports" icon={t("ui_icon_report")} label={t("mason_stat_bugs")} activeTab={activeTab} setTab={setActiveTab} />
           ...
           <HubTabButton id="support_settings" icon={t("ui_icon_support_agent")} label={t("wf_tab_support")} activeTab={activeTab} setTab={setActiveTab} />
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
        <div className="flex flex-col gap-4">
          <div className="theme-glass-panel border border-white/5 rounded-2xl p-6 flex items-center justify-between">
             ...
             <span className="material-symbols-outlined !text-4xl opacity-20">{t("ui_icon_memory")}</span>
          </div>
          ...
          <div className="theme-glass-panel border border-white/5 rounded-2xl p-6 flex flex-col gap-4">
             <span className="text-[10px] font-black uppercase tracking-widest text-[var(--subtext)] opacity-70 flex items-center gap-2">
               <span className="material-symbols-outlined !text-[14px]">{t("ui_icon_speed")}</span> {t("auto_system_load")}
             </span>
          ...
          <div className="theme-glass-panel border border-white/5 rounded-2xl p-6 flex flex-col gap-4">
             <span className="text-[10px] font-black uppercase tracking-widest text-[var(--subtext)] opacity-70 flex items-center gap-2">
               <span className="material-symbols-outlined !text-[14px]">{t("ui_icon_router")}</span> {t("auto_network_diagnostics")}
             </span>
             <div className="grid grid-cols-2 gap-4">
               <div className="flex flex-col gap-1">
                 <span className="text-[9px] uppercase tracking-widest font-bold opacity-50">{t("wf_latency")}</span>
                 <span className="text-lg font-black text-[var(--text)]">{stats.networkLatency || '--'} {t("auto_ms")}</span>
               </div>
               <div className="flex flex-col gap-1">
...
function WayfinderCommandScreen({ setTab, setComplianceFilter, onOpenMasonProfile }: any) {
  ...
  const getNetworkIcon = (status: string, latency: number | null) => {
    if (status !== 'ONLINE') return t("ui_icon_wifi_0");
    if (latency && latency > 500) return t("ui_icon_wifi_1");
    if (latency && latency > 150) return t("ui_icon_wifi_2");
    if (latency && latency > 50) return t("ui_icon_wifi_3");
    return t("ui_icon_wifi_4");
  };
  ...
  return (
    <div className="flex flex-col gap-10 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full pr-4 pb-32 mt-8">
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 w-full">
          <DashboardStatTile icon={<span className="material-symbols-outlined !text-4xl">{t("ui_icon_warning")}</span>} number={stats.defconLevel} label={t("sa_defcon_global")} colorClass={getDefconColor(stats.defconLevel)} onClick={() => setDefconOpen(true)} />
          <DashboardStatTile icon={<span className="material-symbols-outlined !text-4xl">{t("ui_icon_dns")}</span>} number={stats.networkLatency ? `${stats.networkLatency}` : "---"} label={stats.networkStatus === "ONLINE" ? (t("wf_stat_server_nominal")) : (t("wf_stat_server_degraded"))} colorClass={stats.networkStatus === "ONLINE" ? "border-emerald-500/30 text-emerald-500 hover:border-emerald-500 bg-emerald-500/10 hover:bg-emerald-500/20" : "border-yellow-500/30 text-yellow-500 hover:border-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20"} onClick={() => setHealthOpen(true)} />
          <DashboardStatTile icon={<span className="material-symbols-outlined !text-4xl">{t("ui_icon_local_activity")}</span>} number={stats.supportQueue} label={t("wf_stat_support_queue")} colorClass="border-purple-500/30 text-purple-500 hover:border-purple-500 bg-purple-500/10 hover:bg-purple-500/20" onClick={() => setTab("sanctuary_tickets")} />
          <DashboardStatTile icon={<span className="material-symbols-outlined !text-4xl">{t("ui_icon_flag")}</span>} number={stats.flaggedQueue} label={t("wf_stat_flagged")} colorClass="border-orange-500/30 text-orange-500 hover:border-orange-500 bg-orange-500/10 hover:bg-orange-500/20" onClick={() => { if(setComplianceFilter) setComplianceFilter('flagged'); setTab("compliance"); }} />
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 w-full">
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 shadow-lg hover:bg-white/5 hover:border-[var(--accent)]/30 transition-all text-center h-32">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest opacity-70 text-[var(--subtext)] leading-tight">{t("hub_stat_citizens")}</span>
             </div>
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 shadow-lg hover:bg-white/5 hover:border-[var(--accent)]/30 transition-all text-center h-32">
             ...
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 shadow-lg hover:bg-white/5 hover:border-purple-500/30 transition-all text-center h-32">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest opacity-70 text-black-300/70 leading-tight">{t("hub_stat_architects")}</span>
             </div>
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 shadow-lg hover:bg-white/5 hover:border-indigo-500/30 transition-all text-center h-32">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest opacity-70 text-black-300/70 leading-tight">{t("hub_stat_senior_architects")}</span>
             </div>
             <div className="theme-glass-panel border border-white/5 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 shadow-lg hover:bg-white/5 hover:border-red-500/30 transition-all text-center h-32">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest opacity-70 text-black-400/70 leading-tight">{t("hub_tab_malware_logs")}</span>
             </div>
          </div>
        </div>

        <div className="w-[380px] shrink-0 flex flex-col">
           <h2 className="text-xl font-black uppercase tracking-widest text-[var(--text)] mb-6">{t("wf_quick_links")}</h2>
           <div className="flex flex-col gap-4">
             <button onClick={() => setTab("linker")} className="w-full p-6 theme-glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-[1.5rem] hover:bg-white/5 hover:border-[var(--accent)]/50 hover:shadow-[0_0_40px_rgba(var(--accent-rgb),0.1)] transition-all text-left group relative overflow-hidden h-24">
               <div className="flex items-center gap-5 h-full">
                 ...
                 <div className="flex flex-col gap-1 flex-1 min-w-0">
                   ...
                   <span className="text-[8px] uppercase font-bold text-emerald-400 opacity-80 group-hover:text-emerald-300 tracking-widest flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]"></span> {t("sa_link_linker_sub")}</span>
                 </div>
               </div>
             </button>
             ...
             <button onClick={() => setTab("compliance")} className="w-full p-6 theme-glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-[1.5rem] hover:bg-white/5 hover:border-[var(--accent)]/50 hover:shadow-[0_0_40px_rgba(var(--accent-rgb),0.1)] transition-all text-left group relative overflow-hidden h-24">
               ...
               <div className="flex items-center gap-5 h-full relative z-10">
                 ...
                 <div className="flex flex-col gap-1 flex-1 min-w-0">
                   ...
                   <span className="text-[8px] uppercase font-bold text-orange-400 opacity-80 group-hover:text-orange-300 tracking-widest flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-orange-400 shadow-[0_0_8px_rgba(251,146,60,0.8)]"></span> {t("sa_link_compliance_sub")}</span>
                 </div>
               </div>
             </button>
             ...
             <button onClick={() => setTab("sanctuary_tickets")} className="w-full p-6 theme-glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-[1.5rem] hover:bg-white/5 hover:border-[var(--accent)]/50 hover:shadow-[0_0_40px_rgba(var(--accent-rgb),0.1)] transition-all text-left group relative overflow-hidden h-24">
               <div className="flex items-center gap-5 h-full">
                 ...
                 <div className="flex flex-col gap-1 flex-1 min-w-0">
                   ...
                   <span className="text-[8px] uppercase font-bold text-purple-400 opacity-80 group-hover:text-purple-300 tracking-widest flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-purple-400 shadow-[0_0_8px_rgba(168,85,247,0.8)]"></span> {t("wf_link_tickets_sub")}</span>
                 </div>
               </div>
             </button>
             ...
             <button onClick={() => setTab("support_settings")} className="w-full p-6 theme-glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-[1.5rem] hover:bg-white/5 hover:border-[var(--accent)]/50 hover:shadow-[0_0_40px_rgba(var(--accent-rgb),0.1)] transition-all text-left group relative overflow-hidden h-24">
               <div className="flex items-center gap-5 h-full">
                 <div className="w-12 h-12 rounded-xl theme-glass-inner border border-white/10 flex items-center justify-center shrink-0 group-hover:border-[var(--accent)]/30 transition-colors">
                   <span className="material-symbols-outlined !text-3xl opacity-70 group-hover:scale-110 group-hover:opacity-100 transition-all duration-300 drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]">{t("ui_icon_support_agent")}</span>
                 </div>
                 <div className="flex flex-col gap-1 flex-1 min-w-0">
                   <h3 className="text-[11px] font-black uppercase tracking-widest text-[var(--text)] group-hover:text-[var(--accent)] transition-colors truncate">{t("sa_title_support_settings")}</h3>
                   <span className="text-[8px] uppercase font-bold text-amber-400 opacity-80 group-hover:text-amber-300 tracking-widest flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.8)]"></span> {t("wf_link_support_sub")}</span>
                 </div>
               </div>
             </button>
             ...
             <button onClick={() => setTab("audit_logs")} className="w-full p-6 theme-glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-[1.5rem] hover:bg-white/5 hover:border-[var(--accent)]/50 hover:shadow-[0_0_40px_rgba(var(--accent-rgb),0.1)] transition-all text-left group relative overflow-hidden h-24">
               <div className="flex items-center gap-5 h-full">
                 ...
                 <div className="flex flex-col gap-1 flex-1 min-w-0">
                   ...
                   <span className="text-[8px] uppercase font-bold text-blue-400 opacity-80 group-hover:text-blue-300 tracking-widest flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.8)]"></span> {t("sa_link_audit_sub")}</span>
                 </div>
               </div>
             </button>
           </div>
