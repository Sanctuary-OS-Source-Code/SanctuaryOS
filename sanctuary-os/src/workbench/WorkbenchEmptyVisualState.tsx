...
export const WorkbenchEmptyVisualState: React.FC<WorkbenchEmptyVisualStateProps> = ({ t, selectedFile }) => (
   <div className="flex-1 flex flex-col items-center justify-center py-20 gap-8 min-h-[400px]">
      ...
      <div className="mt-4 flex items-center gap-4">
         <button onClick={() => { window.location.href = '#/nexus?tab=templates&q=' + encodeURIComponent(selectedFile?.name || ''); }} className="mt-8 h-12 px-6 rounded-2xl bg-[var(--accent)]/[15%] border border-[var(--accent)]/[30%] text-[var(--accent)] hover:bg-[var(--accent)]/[25%] font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:scale-105 transition-all shadow-[0_10px_30px_rgba(var(--accent-rgb),0.1)]">
