  ...
  if (!settings || settings.length === 0) {
      return (
          <div className="flex-1 flex flex-col items-center justify-center py-20 gap-8 min-h-[400px]">
             <div className="w-24 h-24 rounded-full glass-panel flex items-center justify-center border border-[color-mix(in_srgb,var(--text)_10%,transparent)] opacity-40 shadow-inner">
             ...
             <div className="flex flex-col items-center gap-3 text-center opacity-60">
                <span className="text-[14px] font-black capitalize tracking-[0.2em] text-[var(--text)]">{t("no_visual_template")}</span>
             </div>
          </div>
      );
  }
                 ...
                 <div key={idx} className="glass-panel border border-[color-mix(in_srgb,var(--text)_10%,transparent)] rounded-[var(--radius)] p-6 shadow-inner flex flex-col xl:flex-row xl:items-center justify-start group hover:border-[color-mix(in_srgb,var(--text)_30%,transparent)] transition-all duration-300 gap-6 opacity-80 pointer-events-none select-none">
                    ...
                    <div className="shrink-0 flex items-center justify-end">
                       {setting.type === "boolean" && (
                          <div className={`w-14 h-8 rounded-full transition-all duration-300 relative flex items-center px-1 border shadow-inner ${val ? 'bg-[color-mix(in_srgb,var(--accent)_20%,transparent)] border-[color-mix(in_srgb,var(--accent)_30%,transparent)]' : 'glass-surface border-[color-mix(in_srgb,var(--text)_10%,transparent)]'}`}>
                       ...
                       {setting.type === "string" && (
                          <div className="relative">
                             <input type="text" value={val || ""} readOnly className="glass-surface border border-[color-mix(in_srgb,var(--text)_10%,transparent)] rounded-xl h-10 px-4 text-[12px] font-bold text-[var(--text)] focus:outline-none w-48 truncate" />
                          </div>
                       )}
                       {setting.type === "options" && (
                          <div className="glass-surface border border-[color-mix(in_srgb,var(--text)_10%,transparent)] rounded-xl h-10 px-4 text-[12px] font-bold text-[var(--text)] flex items-center justify-start w-48">
                             ...
                             <span className="material-symbols-outlined !text-[16px] text-[var(--subtext)]">{t("icon_expand_more")}</span>
                          </div>
                       )}
                    </div>
                 </div>
