                 ...
                 <div key={idx} className="glass-panel border border-[color-mix(in_srgb,var(--text)_10%,transparent)] rounded-[var(--radius)] p-6 shadow-inner flex flex-col xl:flex-row xl:items-center justify-start group hover:border-[color-mix(in_srgb,var(--text)_30%,transparent)] transition-all duration-300 gap-6 opacity-80 pointer-events-none select-none">
                    <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                       <div className="flex flex-wrap items-center gap-3">
                          ...
                          {setting.risk === "advanced" && (
                             <span className="px-2.5 py-0.5 rounded-full bg-red-500/[15%] text-[var(--danger)] text-[8px] font-black tracking-widest uppercase border border-red-500/[30%]">{t("advanced_badge") || "ADVANCED"}</span>
                          )}
                       </div>
                    ...
                    <div className="shrink-0 flex items-center justify-end">
                       {setting.type === "boolean" && (
                          <div className={`w-14 h-8 rounded-full transition-all duration-300 relative flex items-center px-1 border shadow-inner ${val ? 'bg-[var(--accent)]/[20%] border-[var(--accent)]/[30%]' : 'glass-surface border-[color-mix(in_srgb,var(--text)_10%,transparent)]'}`}>
