...
import { isVersionMatch, deriveHumanReadableVersion, getHighestVersion, CustomComplianceDropdown, extractPostImage, stripMarkdown } from "./shared";
...
function MasonPostsEditor({ masonId, masonProfileId, handleOpenMasonProfile }: { masonId: string, masonProfileId: string, handleOpenMasonProfile?: (masonId: string, postId?: string) => void }) {
  ...
  const fetchPostsAndAssets = async () => {
    ...
    const { data: marketAssetsData } = await supabase.from('marketplace_assets').select('id, name, asset_type').ilike('author', mName);
...
function MasonMarketplace({ masonProfile }: { masonProfile: any }) {
  ...
  const fetchAssets = async () => {
    setLoading(true);
    const { data } = await supabase.from('marketplace_assets').select('*').ilike('author', masonProfile.name).order('created_at', { ascending: false });
    if (data) {
      setAssets(data);
