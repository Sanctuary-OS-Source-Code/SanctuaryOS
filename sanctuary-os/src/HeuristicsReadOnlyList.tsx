      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredSignatures.map(sig => {
            ...
            const bgClass = isMalware ? 'bg-[color-mix(in_srgb,var(--danger)_10%,transparent)]' : isExplicit ? 'bg-[#fbbf24]/10' : 'bg-[color-mix(in_srgb,var(--accent)_10%,transparent)]';
            ...
            return (
              <UniversalCard
                ...
                badges={[
                  <span key="rating" className={`px-3 py-1.5 rounded-lg text-[9px] font-black tracking-widest capitalize border shadow-inner shrink-0 transition-colors bg-[color-mix(in_srgb,var(--text)_5%,transparent)] ${bgClass} ${sColor} ${sBorder}/20`}>
                    {sig.severity}
                  </span>
                ]}
                footer={
                  <div className="flex justify-start items-center w-full">
                    ...
                    <span className="text-[10px] font-black theme-text-accent capitalize opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0">{t("ui_edit_metadata")} &rarr;</span>
                  </div>
                }
              />
            );
