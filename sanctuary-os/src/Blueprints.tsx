...
export default function Blueprints({
  ...
  const renderVaultCard = (set: any) => {
    ...
    return (
      <div key={set.name} className={`theme-glass-panel p-5 rounded-[var(--radius)] flex flex-col transition-all min-h-[14rem] shadow-xl relative overflow-hidden group/card ${activeSetName === set.name ? 'border-[color-mix(in_srgb,var(--success)_30%,transparent)] shadow-[0_0_40px_rgba(var(--success-rgb),0.1)] bg-[color-mix(in_srgb,var(--success)_2%,transparent)]' : 'border-[color-mix(in_srgb,var(--text)_5%,transparent)] hover:border-[color-mix(in_srgb,var(--text)_15%,transparent)] hover:shadow-[0_0_30px_rgba(255,255,255,0.02)] hover:bg-black/5'}`}>
