...
export default function NexusManager() {
  ...
  return (
    <div style={{ color: 'var(--text)', fontFamily: 'monospace' }}>
      <h2 style={{ color: 'var(--accent)', marginBottom: '5px' }}>{t("nexus_title") || "THE NEXUS"}</h2>
      <p style={{ color: 'var(--subtext)', marginBottom: '30px', fontSize: '0.9rem' }}>{t("nexus_subtitle") || "Global Directive Management & Cloud Logic"}</p>
      ...
      <div style={{ background: 'var(--sidebar)', padding: '25px', borderRadius: '12px', marginBottom: '40px', border: '1px solid rgba(255,255,255,0.05)' }}>
        <h3 style={{ marginTop: 0, color: 'var(--warning)', fontSize: '1rem', marginBottom: '20px' }}>
          {t("nexus_forge_title") || "NEW DIRECTIVE"}
        </h3>
        <form onSubmit={handleAddGhost} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ display: 'flex', gap: '15px' }}>
            <input placeholder={t("nexus_enemy_a") || "ARTIFACT A"} value={modA} onChange={(e) => setModA(e.target.value)} required style={inputStyle} />
            <div style={{ alignSelf: 'center', color: 'var(--subtext)', fontWeight: 'bold' }}>{t("nexus_icon_vs")}</div>
            <input placeholder={t("nexus_enemy_b") || "ARTIFACT B"} value={modB} onChange={(e) => setModB(e.target.value)} required style={inputStyle} />
            <select value={severity} onChange={(e) => setSeverity(Number(e.target.value))} style={selectStyle}>
              <option value={4}>{t("nexus_tier4") || "Severity 4: Fatal Collision"}</option>
              <option value={3}>{t("nexus_tier3") || "Severity 3: Tuning Overlap"}</option>
            </select>
          </div>
          <textarea placeholder={t("nexus_resolution") || "Resolution Directive (What should the citizen do?)"} value={note} onChange={(e) => setNote(e.target.value)} style={{ ...inputStyle, height: '80px', resize: 'none' }} />
          <button type="submit" style={submitBtnStyle}>{t("nexus_inject") || "INJECT DIRECTIVE INTO REGISTRY"}</button>
        </form>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', color: 'var(--accent)' }}>{t("nexus_syncing") || "SYNCING WITH REGISTRY..."}</div>
        ) : (
          ghosts.map((g) => (
            <div key={g.id} style={{ background: 'var(--sidebar)', borderRadius: '8px', borderLeft: `4px solid ${g.severity_rank === 4 ? 'var(--danger)' : 'var(--warning)'}`, overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center', fontSize: '1.1rem', fontWeight: 'bold' }}>
                  ...
                  <span style={{ color: 'var(--danger)', fontSize: '0.8rem' }}>{t("nexus_vs") || "VS"}</span>
                ...
                <div style={{ padding: '4px 12px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold', background: g.severity_rank === 4 ? 'var(--danger)' : 'var(--warning)', color: 'white' }}>
                  {t("nexus_rank") || "RANK"}{g.severity_rank}
                </div>
              </div>

              <div style={{ padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '30px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.7rem', color: 'var(--subtext)', marginBottom: '8px', letterSpacing: '1px' }}>{t("nexus_sys_directive") || "SYSTEM DIRECTIVE:"}</div>
                  <div style={{ color: 'var(--success)', lineHeight: '1.5', fontSize: '0.95rem' }}>{g.resolution_note}</div>
                </div>
                <button onClick={() => handleDeleteGhost(g.id)} style={purgeBtnStyle}>{t("nexus_purge") || "DELETE"}</button>
              </div>
            </div>
          ))
        )}
