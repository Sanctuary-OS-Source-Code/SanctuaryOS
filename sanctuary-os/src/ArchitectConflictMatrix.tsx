          ...
          <div className="flex flex-col gap-10">
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
                {filteredPendingGhosts.slice(0, 4).length > 0 ? filteredPendingGhosts.slice(0, 4).map(renderConflictCard) : (
                  <EmptyState icon="check_circle" title={t("no_pending_conflicts")} className="col-span-full py-8" />
                )}
              </div>
            </div>
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
                {filteredActiveGhosts.slice(0, 4).length > 0 ? filteredActiveGhosts.slice(0, 4).map(renderConflictCard) : (
                  <EmptyState icon="history" title={t("no_history")} className="col-span-full py-8" />
                )}
              </div>
            </div>
          </div>
        ...
        {filterTab === 'pending' && (
          <div className="mb-4">
            <div className="grid grid-cols-[repeat(auto-fill,minmax(350px,1fr))] gap-6">
              ...
              ) : filteredPendingGhosts.slice(0, visibleCount).map(renderConflictCard)}
            </div>
            ...
            headerActions={
              <PanelHeaderGroup>
                ...
                {editConflictId && !isDeleting && (
                  <PanelHeaderButton icon="delete" variant="danger" tooltip={editingGhost?.status === 'pending' ? (t("matrix_btn_reject")) : (t("purge"))} onClick={() => setIsDeleting(true)} />
                )}
                {editingGhost?.status !== 'pending' && !isDeleting && (
                  <PanelHeaderButton icon="check" variant="success" disabled={!modA || !modB || (!!editConflictId && !updateReason.trim())} tooltip={editConflictId ? (t("edit_side_panel")) : (t("inject"))} onClick={() => handleAddGhost()} />
                )}
                {editingGhost?.status === 'pending' && !isDeleting && (
                  <PanelHeaderButton icon="check" variant="success" tooltip={t("ui_btn_approve")} onClick={() => handleConfirmSidePanelApprove()} />
                )}
              </PanelHeaderGroup>
            }
            noPadding={true}
