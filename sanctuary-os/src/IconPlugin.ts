      ...
      new Plugin({
        ...
        props: {
          ...
          handleKeyDown(view, event) {
            ...
            // Get text before and after cursor within the current node
...
function getDecorations(doc: any) {
  ...
  doc.descendants((node: any, pos: number) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      // Hide the text [ICON:name]
      decorations.push(
        Decoration.inline(start, end, {
          nodeName: 'span',
          style: 'font-size: 0; color: transparent; user-select: none;', // Set user-select to none so it's not half-selectable
          class: 'icon-tag-hidden',
        })
      );
      
      // Inject the widget right after to display the icon visually
      decorations.push(
