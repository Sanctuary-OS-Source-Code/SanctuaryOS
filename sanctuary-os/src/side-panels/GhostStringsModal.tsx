...
import { HoverTooltip, cleanSearchName, SidePanel, handleOpenUrl, getModFallbackUrl, getModFallbackAuthor } from "../shared";
          ...
          const renderGhostList = (modList: string[]) => (
            <div className="flex flex-col gap-3">
              {modList.map((mod: string, idx: number) => {
                ...
                const playSet = useStore.getState().playSets.find((s: any) => s.name === setName);
                const modHash = playSet?.modHashes?.[mod];
                const blueprintMeta = playSet?.modMeta?.[mod];
                const activeSchema = useStore.getState().activeGameSchema;
                const targetUrl = getModFallbackUrl(mod, modHash, activeSchema, blueprintMeta);
                const customAuthor = getModFallbackAuthor(modHash, blueprintMeta, mod, activeSchema);
                
                return (
                  <UniversalCard
                    key={idx}
                    layout="vertical-compact"
                    title={cleanSearchName(mod, activeSchema)}
                    subtitle={customAuthor ? `By ${customAuthor}` : (mod.split('/').pop() || mod)}
                    badges={
