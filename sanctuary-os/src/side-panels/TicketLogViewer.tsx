...
export default function TicketLogViewer({
 logs }: { logs: string }) {
  ...
  const handleImportBlueprint = () => {
      ...
      let newName = blueprintJson.name || t("support_imported_blueprint") || "Imported Blueprint";
      let counter = 1;
      while (copySets.some((s: any) => s.name.toLowerCase() === newName.toLowerCase())) {
          newName = `${blueprintJson.name || t("support_imported_blueprint") || "Imported Blueprint"} (${counter})`;
          counter++;
      }
    ...
    <div className="flex flex-col w-full mt-4 gap-4">
       ...
       {blueprintJson && (
          <SidePanel
            ...
            subtitle={blueprintJson.name || t("support_import_load_order") || "Import Load Order"}
            icon={t("icon_map")}
