            ...
            return (
              <div key={idx} className="flex justify-start items-center glass-surface border border-[color-mix(in_srgb,var(--text)_5%,transparent)] p-4 rounded-2xl hover:border-[color-mix(in_srgb,var(--text)_20%,transparent)] transition-all group shadow-md">
                <div className="flex flex-col min-w-0 pr-4">
                  <span className="text-xs font-black text-[var(--text)] capitalize truncate group-hover:theme-text-accent transition-colors">{cleanSearchName(mod.name, useStore.getState().activeGameSchema)}</span>
                  <span className="text-[9px] font-bold text-[var(--subtext)] opacity-60 capitalize tracking-widest mt-1">{mod.author || t("unknown_mason")}</span>
