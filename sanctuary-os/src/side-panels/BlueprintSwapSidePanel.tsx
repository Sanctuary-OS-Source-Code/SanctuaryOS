                ...
                >
                  ...
                  <div className="relative p-6 flex flex-col sm:flex-row items-center gap-6 z-10">
                    <div className={`w-16 h-16 rounded-[1.25rem] flex items-center justify-center shrink-0 border transition-all duration-500 shadow-inner ${
                      isActive 
                        ? 'border-[color-mix(in_srgb,var(--accent)_40%,transparent)] bg-[color-mix(in_srgb,var(--accent)_20%,transparent)] backdrop-blur-xl shadow-md' 
                      ...
                      <span className={`material-symbols-outlined transition-colors duration-500 ${isActive ? 'theme-text-accent drop-shadow-md' : 'text-[var(--text)] opacity-50 group-hover:opacity-100 group-hover:theme-text-accent'}`}>
                        {t("icon_map")}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col min-w-0 w-full">
                      <div className="flex items-center justify-start gap-4 mb-2">
                        <h4 className={`text-xl font-black capitalize tracking-widest truncate transition-colors duration-500 ${isActive ? 'theme-text-accent drop-shadow-md' : 'text-[var(--text)] group-hover:theme-text-accent'}`}>
                          {bp.name}
                        </h4>
                        {isActive && (
                          <span className="px-4 py-1.5 rounded-full bg-[color-mix(in_srgb,var(--accent)_20%,transparent)] border border-[color-mix(in_srgb,var(--accent)_40%,transparent)] backdrop-blur-xl theme-text-accent text-[9px] font-black capitalize tracking-[0.2em] shadow-md animate-[pulse_2s_ease-in-out_infinite] shrink-0 flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-4 mt-1">
                        <span className={`text-[10px] font-black capitalize tracking-widest flex items-center gap-2 transition-colors ${isActive ? 'theme-text-accent opacity-80' : 'text-[var(--subtext)] opacity-50'}`}>
