    ...
    >
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
        <div className="flex flex-col gap-4">
          <div className="glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-2xl p-6 flex items-center justify-start">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black capitalize tracking-widest text-[var(--subtext)] opacity-70">{t("wf_core_node")}</span>
            ...
            <span className="material-symbols-outlined opacity-20">{t("icon_memory")}</span>
          </div>

          <div className="glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-2xl p-6 flex flex-col gap-4">
            <span className="text-[10px] font-black capitalize tracking-widest text-[var(--subtext)] opacity-70 flex items-center gap-2">
              <span className="material-symbols-outlined !text-[14px]">{t("icon_speed")}</span> {t("auto_system_load")}
            </span>
            <div className="flex flex-col gap-2">
              <div className="flex justify-start text-xs font-bold text-[var(--text)]">
                <span>{t("wf_health_cpu_usage")}</span>
                <span className="text-[var(--accent)]">{t("auto_42")}</span>
              </div>
              <div className="w-full h-2 bg-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-full overflow-hidden">
          ...
          <div className="glass-panel border border-[color-mix(in_srgb,var(--text)_5%,transparent)] rounded-2xl p-6 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <span className="text-[9px] capitalize tracking-widest font-bold opacity-50">{t("wf_latency")}</span>
                <span className="text-lg font-black text-[var(--text)]">{stats.networkLatency || '--'} {t("icon_ms")}</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-[9px] capitalize tracking-widest font-bold opacity-50">{t("wf_connections")}</span>
                <span className="text-lg font-black text-blue-400">{t("auto_1_204")}</span>
              </div>
              <div className="flex flex-col gap-1 col-span-2 mt-2">
                <span className="text-[9px] capitalize tracking-widest font-bold opacity-50">{t("wf_db_sync")}</span>
