        ...
        className="flex flex-col items-start gap-4 p-6 glass-panel rounded-[var(--radius)] hover:border-[color-mix(in_srgb,var(--accent)_50%,transparent)] hover:bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] hover:shadow-[0_10px_30px_rgba(var(--accent-rgb),0.15)] transition-all duration-300 group text-left border border-[color-mix(in_srgb,var(--text)_5%,transparent)] relative w-full"
              >
                ...
                <div className="flex items-center justify-start w-full relative z-10">
                  ...
                  <span className="text-[9px] font-black capitalize tracking-widest text-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_15%,transparent)] border border-[color-mix(in_srgb,var(--accent)_30%,transparent)] px-3 py-1.5 rounded-full shadow-inner">{template.target_file}</span>
                </div>
                ...
                <div className="flex items-center justify-start w-full mt-2 relative z-10 border-t border-[color-mix(in_srgb,var(--text)_5%,transparent)] pt-4">
                   <div className="flex items-center gap-6">
                       {template.mod_author && (
                         <div className="flex items-center gap-2">
                           ...
                           <span className="text-[10px] capitalize font-bold tracking-widest text-[var(--text)] opacity-80">{t("author_author_label")} <span className="text-[var(--accent)]">{template.mod_author}</span></span>
                         </div>
                       )}
                       {template.template_version && (
                         <div className="flex items-center gap-2">
                           <span className="material-symbols-outlined !text-[14px] text-[var(--subtext)]">{t("icon_tag")}</span>
                           <span className="text-[10px] capitalize font-bold tracking-widest text-[var(--text)] opacity-80">{t("author_version_label")} <span className="text-[var(--accent)]">{template.template_version}</span></span>
                         </div>
                       )}
                   </div>
                   <div className="flex items-center gap-2 text-[var(--accent)] opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0 duration-300">
                     <span className="text-[9px] font-black capitalize tracking-widest">{t("author_import_to_editor")}</span>
