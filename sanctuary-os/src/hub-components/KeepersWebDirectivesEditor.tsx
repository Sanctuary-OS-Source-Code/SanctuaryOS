...
export default function KeepersWebDirectivesEditor({ isSidePanel = false, isOpen = true, onClose = () => {} }: any = {}) {
    ...
    return (
      <div className="grid grid-cols-1 2xl:grid-cols-2 gap-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between gap-4 border-b border-[color-mix(in_srgb,var(--text)_5%,transparent)] pb-4">
            <h3 className="text-sm font-black text-[var(--text)] tracking-widest uppercase flex items-center gap-2">
              <span className="material-symbols-outlined !text-[18px] text-[var(--accent)]">gavel</span>
              {t("recent_active_directives") || "Recent Active Directives"}
            </h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-6">
            ...
            {activePosts.length === 0 && <EmptyState icon="gavel" title={t("no_transmissions")} className="py-8" />}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between gap-4 border-b border-[color-mix(in_srgb,var(--text)_5%,transparent)] pb-4">
            <h3 className="text-sm font-black text-[var(--text)] tracking-widest uppercase flex items-center gap-2">
              <span className="material-symbols-outlined !text-[18px] text-[var(--subtext)]">block</span>
              {t("recent_inactive_directives") || "Recent Inactive Directives"}
            </h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-6">
            {inactivePosts.slice(0, 10).map(renderPostCard)}
            {inactivePosts.length === 0 && <EmptyState icon="gavel" title={t("no_transmissions")} className="py-8" />}
          </div>
        </div>
      </div>
    );
  ...
  const contentBlock = (
    <ElevatedHubLayout
      headerTitle={t("web_directives") || "Web Directives"}
      headerSubtitle={t("web_directives_subtitle") || "Manage website instructions and guides"}
      headerIcon="gavel"
