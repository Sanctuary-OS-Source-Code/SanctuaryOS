...
import { standardAccentGlassButtonClass, standardButtonClass, SidePanel, CustomDropdown, extractPostImage, stripMarkdown, EmptyState, HoverTooltip } from '../shared';
...
export function WayfinderPostsEditor({ authorId, authorProfileId, handleOpenWayfinderProfile, isOversight, isSidePanel, isOpen, onClose }: { authorId: string, authorProfileId: string, handleOpenWayfinderProfile?: (authorId: string, postId?: string) => void, isOversight?: boolean, isSidePanel?: boolean, isOpen?: boolean, onClose?: () => void }) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      IconPlugin,
      Image,
      Link.configure({ HTMLAttributes: { target: '_blank', rel: 'noopener noreferrer' }, protocols: ['asset'] })
    ],
    content: content,
    onUpdate: ({ editor }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let payload: any = { title: title.trim(), description: description.trim() || null, message: content.trim(), category: category, target_audience: targetAudience.join(','), code_snippet: codeSnippet.trim() || null, is_pinned: isPinned ? "true" : "false", is_active: isActive };
        ...
        <div className="relative flex-1 max-w-4xl ml-auto flex gap-4 items-center justify-end">
          ...
          <div className="flex items-stretch overflow-hidden theme-glass-panel rounded-xl divide-x divide-white/5 border border-white/5 h-12 shrink-0 z-40">
            <button
              ...
              className={`h-full px-5 rounded-none flex items-center justify-center text-[10px] font-black uppercase tracking-widest transition-all ${filterStatus === 'Active' ? 'bg-[var(--accent)]/20 text-[var(--accent)]' : 'text-[var(--subtext)] hover:text-[var(--text)] hover:bg-white/5'}`}
            >
              {t("masonhub_status_active")}
            </button>
            <button
              onClick={() => setFilterStatus("Inactive")}
              className={`h-full px-5 rounded-none flex items-center justify-center text-[10px] font-black uppercase tracking-widest transition-all ${filterStatus === 'Inactive' ? 'bg-[var(--accent)]/20 text-[var(--accent)]' : 'text-[var(--subtext)] hover:text-[var(--text)] hover:bg-white/5'}`}
            >
              {t("status_inactive")}
            </button>
          </div>
          ...
          >
            <div className="flex flex-col gap-6">
              ...
              <div className="flex border-b border-[color-mix(in_srgb,var(--text)_5%,transparent)] mb-2 p-1 bg-black/40 rounded-2xl theme-glass-inner">
                ...
                <div className="ml-4 pr-4 flex items-center gap-6">
