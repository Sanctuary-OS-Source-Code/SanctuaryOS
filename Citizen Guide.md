...
- **Auto-Detect Paths:** Click the Cloud icon to automatically locate your game and document folders for your active workspace.
  - **Bin Folder (Live Path):** The directory where your game executable lives. *V5 Update:* Sanctuary no longer relies on the notoriously unreliable `GameVersion.txt`. It now natively extracts the genuine patch version directly from your active game executable (e.g., `TS4_x64.exe`) using an in-process Rust PE parser. This ensures AV compliance by avoiding subprocess overhead. If the target executable is unspecified, the parser dynamically scans the directory while strictly filtering out crash-handlers, redistributables, and anti-cheat binaries. If the OS metadata isn't available, it falls back to the schema-defined `.ini` manifest to guarantee 100% engine version accuracy.
