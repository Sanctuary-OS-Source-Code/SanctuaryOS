...
- **Engine Version Recognition:** Sanctuary has deprecated `GameVersion.txt`. It now natively extracts genuine patch version metadata directly from the active game executable (e.g. `.exe` file properties) using an in-process Rust PE parser. This ensures AV compliance by avoiding subprocess overhead. If the target executable is unspecified, the parser dynamically scans the directory while strictly filtering out crash-handlers, redistributables, and anti-cheat binaries. If native OS metadata is unavailable, it gracefully falls back to parsing a schema-defined `.ini` manifest to guarantee version accuracy and prevent "Update-Day Disasters."
...
- **The Nuclear Override:** Located in System Preferences, this one-time toggle allows a user to bypass security lockouts. Engaging this explicitly records the user's hardware signature hash into the decoupled `hardware_bans` ledger within the Core OS database, instantly and permanently severing all communication with the Sanctuary Cloud without requiring continuous hardware tracking on standard user profiles.
