...
Here is your complete User Guide to mastering this entirely new way of modding.
...
Network
Local Network Only (Offline Mode) & Show Artifact Images (All External Images).
Notifications
...
Highlander Enforcement (Flavors), Nuclear Family Cohesion (Twins), Dependency Chain-Link, and Conflict Intercept (Conflict Tier 4).
Malware Protocol
...
Trust the Airgap: Never put files directly in your "Electronic Arts" Mods folder. Drop them into Sanctuary and let the OS handle the symlinks.
Missing DLC/Versions: Pay attention to the Addon Drawer. If a mod card has a red "Broken" icon instead of a plus sign, hover over it. The tooltip will dynamically tell you exactly what is wrong (e.g., MISSING DLC: [Packs], VERSION MISMATCH, or MISSING ARTIFACT).
Follow Masons: Click a creator's name in The Nexus to view their new Mason Profile and click Follow. Their updates will now securely appear in your "Following" tab inside the Comm-Link.
Check your Status: If your Command Center tile is Amber or Red, run a Radar Sweep and follow the Citizen Action Recommended Alert.
Watch the Launch Button: Green is all clear. If it’s Red, that means a Patch Update is currently rolling out.
Account Not Required: The offline version of Sanctuary OS does not require an account. 
